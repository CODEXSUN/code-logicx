...
import { requireEnvNumber, requireEnvValue } from "@codelogicx/framework/env";
...
const platformSourceRoots = [
  ...
  resolve(repositoryDir, "apps/codelogicx/web")
];
...
export default defineConfig(({ command, mode }) => {
  ...
  return {
    ...
    ...(command === "serve"
      ? {
          server: {
            allowedHosts: ["codelogicx.test", "localhost", "127.0.0.1"],
            fs: {
...
function reactRefreshPreamble() {
  return {
    ...
    name: "codelogicx:react-refresh-preamble",
    transformIndexHtml: {
      handler: () => [
        {
          attrs: { type: "module" },

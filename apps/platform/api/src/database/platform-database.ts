...
import { migrateCodeLogicXDatabase } from "@codelogicx/codelogicx-api";
...
export function platformDatabaseName() {
  return assertDatabaseName(env.DB_NAME, "CodeLogicX database name");
}
...
export async function bootstrapPlatformDatabase() {
  if (bootstrapped || process.env.CODELOGICX_DEV_SKIP_DB === "1") return;
  if (env.CODELOGICX_DB_FRESH_ON_START === "1") {
    const sessionFile = process.env.CODELOGICX_DB_FRESH_SESSION_FILE;
  ...
  bootstrapped = true;
  console.info(`[database] CodeLogicX database ready: "${platformDatabaseName()}"`);
}
...
function assertDestructiveDatabaseAction() {
  if (env.CODELOGICX_DB_RESET_CONFIRM !== "DROP_DATABASE") {
    throw new Error(
      "Set CODELOGICX_DB_RESET_CONFIRM=DROP_DATABASE to reset the CodeLogicX database."
    );
  }
  if (env.NODE_ENV === "production" && env.CODELOGICX_ALLOW_PRODUCTION_DB_RESET !== "1") {

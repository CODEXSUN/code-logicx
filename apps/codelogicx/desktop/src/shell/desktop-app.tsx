...
export const activities = [
  ...
  { icon: Cloud, id: "cloud", label: "Cloud workspace" },
...
export function DesktopApp() {
  ...
  const [theme, setTheme] = useState<Theme>(() => {
    const saved = localStorage.getItem("codelogicx-theme");
  ...
  useEffect(() => {
    ...
    localStorage.setItem("codelogicx-theme", theme);
  }, [resolvedTheme, theme]);
  ...
  useEffect(() => {
    ...
    window.addEventListener("codelogicx:open-terminal", openTerminal);
    return () => window.removeEventListener("codelogicx:open-terminal", openTerminal);
  }, [ui.setTerminalOpen]);
  ...
  if (cloudOnly)
    return (
      <Suspense fallback={<div className="workspace-loading">Loading cloud workspace...</div>}>
        <CloudWorkspace onExitLocal={() => setCloudOnly(false)} />
      </Suspense>
    );
      ...
      >
        ...
        >
          ...
          {ui.activity === "cloud" ? (
            <div className="workspace-surface">
              <Suspense
                fallback={<div className="workspace-loading">Loading cloud workspace...</div>}
              >
                <CloudWorkspace />
              </Suspense>
            </div>
          ...
          {ui.activity === "sync" ? (
            <div className="workspace-surface">
              <Suspense fallback={<div className="workspace-loading">Loading sync...</div>}>
                <PortalWorkspace
                  description="Connect local work data to CodeLogicX Cloud and control synchronization."
                  path="/app/codelogicx/project-sync"

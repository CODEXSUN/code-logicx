import { Cloud, Save, ShieldCheck, UserRound } from "lucide-react";
...
export function DesktopSetup({
  ...
  async function continueToWorkspace() {
    ...
    setError(undefined);
    try {
      await desktopClient.saveDesktopProfile({
        displayName,
        email: email || null,
        rememberIdentity,
        confirmOnStartup
  ...
  return (
    <main className="desktop-setup">
      <section className="desktop-setup-copy">
        ...
        <h1>Who is using this CodeLogicX?</h1>
        <p>
          Your identity stays in this computer’s local SQLite database. You set work groups and
          repository connections on the next page.
        </p>
      </section>
      <section className="desktop-setup-form" aria-label="Local identity setup">
        ...
        <label className="desktop-setup-check">
          <input
          ...
          <span>
            ...
            <small>Use this local identity and recent workspace automatically.</small>
          </span>
        </label>
        <label className="desktop-setup-check">
          <input
            checked={confirmOnStartup}
            disabled={!rememberIdentity}
            onChange={(event) => setConfirmOnStartup(event.target.checked)}
            type="checkbox"
          />
          <span>
            <strong>Ask before restoring a workspace</strong>
            <small>Recommended on shared computers.</small>
          </span>
        </label>
        {error ? (
        ...
        <button className="desktop-setup-cloud" onClick={onOpenCloud} type="button">
          <Cloud size={17} />
          Connect to CodeLogicX Cloud
        </button>
        <p className="desktop-setup-note">
          <ShieldCheck size={15} /> Repository code and model credentials are configured separately.
        </p>
      </section>
    </main>
  );

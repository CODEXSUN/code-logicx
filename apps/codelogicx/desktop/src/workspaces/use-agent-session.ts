...
export function useAgentSession({ onRefreshChanges }: { onRefreshChanges: () => Promise<void> }) {
  ...
  watchdogRef.current ??= new AgentTurnWatchdog({
    ...
    onTimeout: () => {
      setStalled(false);
      setError(
        "The agent produced no activity for three minutes, so CodeLogicX stopped the turn. Send a follow-up to continue."
      );
  ...
  function handleAgentEvent(message: AgentProtocolMessage) {
    ...
    if (
      ...
      setApproval({
        ...
        reason: textAt(message, "params", "reason") ?? "CodeLogicX needs permission to continue."
      });
    }

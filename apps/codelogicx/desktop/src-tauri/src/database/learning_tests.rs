...
fn requires_review_and_marks_missing_evidence_stale() {
    let path = std::env::temp_dir().join(format!(
        "codelogicx-project-learning-{}.db",
        uuid::Uuid::new_v4()
    ));
    ...
    database
        .replace_detected_learnings("C:/work/codelogicx", &detected)
        .expect("detect project fact");
    let summary = database
        .project_learning_summary("C:/work/codelogicx")
        .expect("summarize project facts");
    assert_eq!(summary.candidate_count, 1);
    assert!(database
        .approved_project_learning_context("C:/work/codelogicx")
        .unwrap()
        .is_empty());

    database
        .review_project_learning("C:/work/codelogicx", summary.items[0].id, "approved")
    ...
    database
        .replace_detected_learnings("C:/work/codelogicx", &[])
    ...
    let settings = database
        .save_project_learning_settings("C:/work/codelogicx", false, false)
        .unwrap();

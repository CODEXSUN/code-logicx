...
mod tests {
    ...
    fn scans_only_direct_git_repositories() {
        let root = std::env::temp_dir().join(format!("codelogicx-work-group-{}", uuid::Uuid::new_v4()));
    ...
    fn accepts_github_https_and_ssh_urls() {
        assert_eq!(
            github_repository_name("https://github.com/CODEXSUN/codelogicx.git").expect("HTTPS URL"),
            "codelogicx"
        );
        assert_eq!(

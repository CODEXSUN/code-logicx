...
pub fn start_agent_runtime(
    ...
    let mut child = command.spawn().map_err(|error| {
        DesktopError::Policy(format!(
            "CodeLogicX could not start its Codex engine. Set CODELOGICX_CODEX_BIN to override it. {error}"
        ))
    })?;
    let stdout = child
        .stdout
...
fn prepend_runtime_directory_to_path(command: &mut Command, executable: &str) -> DesktopResult<()> {
    ...
    let path = std::env::join_paths(paths).map_err(|error| {
        DesktopError::Policy(format!(
            "CodeLogicX could not prepare the Codex runtime path. {error}"
        ))
    })?;
...
fn resolve_codex_executable() -> String {
    if let Ok(path) = std::env::var("CODELOGICX_CODEX_BIN") {
        return path;
    }
...
pub fn start_agent_thread(state: State<'_, DesktopState>) -> DesktopResult<u64> {
    ...
    send_request(
        ...
        json!({
            ...
            "serviceName": "codelogicx_desktop"
        }),
    )
}
...
pub fn run_opencode_task(
    ...
    let executable = std::env::var("CODELOGICX_OPENCODE_BIN").unwrap_or_else(|_| "opencode".into());
    let output = Command::new(&executable)
        ...
        .output()
        .map_err(|error| DesktopError::Policy(format!("CodeLogicX could not start OpenCode. Install opencode-ai or set CODELOGICX_OPENCODE_BIN. {error}")))?;
...
fn task_execution_root(state: &State<'_, DesktopState>, task_id: i64) -> DesktopResult<PathBuf> {
    ...
    let managed_root = parent.join(".codelogicx-worktrees").canonicalize()?;

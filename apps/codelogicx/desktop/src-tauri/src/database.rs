...
mod tests {
    ...
    fn persists_agent_tasks_and_messages_per_workspace() {
        let path =
            std::env::temp_dir().join(format!("codelogicx-agent-history-{}.db", uuid::Uuid::new_v4()));
        ...
        let task = database
            .set_agent_task_execution(
                "C:/work/codelogicx",
                task.id,
                "C:/work/.codelogicx-worktrees/task-1",
                "codelogicx/task-1",
            )
            .expect("save task worktree");
        assert_eq!(task.run_status, "ready");
        assert_eq!(task.worktree_branch.as_deref(), Some("codelogicx/task-1"));
        assert_eq!(
            database
                .agent_task_execution_path("C:/work/codelogicx", task.id)
        ...
        assert_eq!(
            database.list_agent_tasks("C:/work/codelogicx").unwrap().len(),
            1
        );
        assert!(database
        ...
        database
            .rename_agent_task("C:/work/codelogicx", task.id, "Startup discussion")
        ...
        assert_eq!(
            database
                .get_agent_task("C:/work/codelogicx", task.id)
    ...
    fn archives_reviews_and_deletes_agent_tasks_per_workspace() {
        let path = std::env::temp_dir().join(format!(
            "codelogicx-agent-task-actions-{}.db",
            uuid::Uuid::new_v4()
        ));
        let mut database = DesktopDatabase::open(path.clone()).expect("open test database");
        let task = database
        ...
        let reviewed = database
            .request_agent_task_review("C:/work/codelogicx", task.id)
            .expect("request review");
        assert!(reviewed.review_requested);
        let renamed = database
            .rename_agent_task("C:/work/codelogicx", task.id, "Release review")
            .expect("rename task");
        ...
        assert!(database
            ...
            .is_err());
        assert!(database
            .archive_agent_task("C:/work/codelogicx", task.id)
            .expect("archive task"));
        assert!(database
            .list_agent_tasks("C:/work/codelogicx")
            .unwrap()
            .is_empty());
        assert!(database
            .delete_agent_task("C:/work/codelogicx", task.id)
            .expect("delete task"));
    ...
    fn persists_local_identity_and_workspace_mapping() {
        let path =
            std::env::temp_dir().join(format!("codelogicx-desktop-setup-{}.db", uuid::Uuid::new_v4()));
        ...
        database
            .save_desktop_workspace(&DesktopWorkspace {
                ...
                project_name: Some("CodeLogicX".into()),
                tagline: None,
                changelog_path: None,
                owner_name: None,
                started_on: None,
    ...
    fn persists_multiple_workspace_folders_with_one_default() {
        let path = std::env::temp_dir().join(format!(
            "codelogicx-work-group-history-{}.db",
            uuid::Uuid::new_v4()
        ));
        let database = DesktopDatabase::open(path.clone()).expect("open test database");
        database
    ...
    fn keeps_workspace_list_order_when_a_workspace_opens() {
        ...
        database
            .mark_workspace_opened("C:/work/codelogicx", "codelogicx")
        ...
        assert_eq!(names, ["cxapp", "cxshop", "codelogicx"]);
        drop(database);
        std::fs::remove_file(path).expect("remove test database");
    }
    ...
    fn persists_one_default_workspace_and_clears_it_when_removed() {
        let path = std::env::temp_dir().join(format!(
            "codelogicx-default-workspace-{}.db",
            uuid::Uuid::new_v4()
        ));
        let mut database = DesktopDatabase::open(path.clone()).expect("open test database");
        for name in ["cxapp", "codelogicx"] {
            database
                .save_desktop_workspace(&DesktopWorkspace {
        ...
        database
            .set_default_desktop_workspace("C:/work/codelogicx")
        ...
        database
            .remove_desktop_workspace("C:/work/codelogicx")
    ...
    fn upgrades_legacy_text_project_ids_without_blocking_desktop_setup() {
        let path = std::env::temp_dir().join(format!(
            "codelogicx-legacy-project-id-{}.db",
            uuid::Uuid::new_v4()
        ));
        let database = DesktopDatabase::open(path.clone()).expect("open test database");
        database
    ...
    fn pins_and_removes_workspace_mappings_without_touching_the_folder() {
        let path = std::env::temp_dir().join(format!(
            "codelogicx-workspace-actions-{}.db",
            uuid::Uuid::new_v4()
        ));
        let database = DesktopDatabase::open(path.clone()).expect("open test database");
        database
    ...
    fn saves_repository_urls_per_work_group() {
        let path =
            std::env::temp_dir().join(format!("codelogicx-repository-url-{}.db", uuid::Uuid::new_v4()));
        let database = DesktopDatabase::open(path.clone()).expect("open test database");

        database
            .save_repository_url(
                "C:/work",
                "https://github.com/CODEXSUN/codelogicx.git",
                "application",
                "project",
            )
        ...
        assert_eq!(urls[0].url, "https://github.com/CODEXSUN/codelogicx.git");
    ...
    fn copies_project_tasks_only_to_registered_projects() {
        let path = std::env::temp_dir().join(format!(
            "codelogicx-project-task-copy-{}.db",
            uuid::Uuid::new_v4()
        ));
        let database = DesktopDatabase::open(path.clone()).expect("open test database");
        let source = database

...
- Rust owns files, Git, terminal processes, Docker, SQLite, and CodeLogicX synchronization.
- The desktop app calls CodeLogicX through the configured public API contract.
- The app does not import Platform or CodeLogicX private source paths.
...
- local SQLite tasks and outbound CodeLogicX synchronization contracts;
...
profile. `CODELOGICX_CODEX_BIN` can override the engine during development. The former
...
CodeLogicX is a transparent desktop client for the bundled Codex App Server. It does not replace
...
approvals, and final answers. CodeLogicX only presents the recorded native state.

...
export class TelegramSupportRepository {
  ...
  async connection() {
    return this.database.selectFrom("codelogicx_telegram_connections").selectAll().orderBy("created_at", "desc").executeTakeFirst();
  }

  async createConnection(token: string) {
    const uuid = randomBytes(8).toString("hex");
    await this.database.insertInto("codelogicx_telegram_connections").values({
  ...
  async mtprotoConnection() {
    return this.database.selectFrom("codelogicx_telegram_connections").selectAll()
  ...
  async saveMtprotoConnection(input: { encryptedSession: string; id: string; username: string; displayName: string }) {
    await this.database.updateTable("codelogicx_telegram_connections")
  ...
  async connect(token: string, chatId: string, username: string, displayName: string) {
    const result = await this.database.updateTable("codelogicx_telegram_connections").set({
  ...
  async disconnect() {
    await this.database.updateTable("codelogicx_telegram_connections").set({ status: "disconnected", updated_at: new Date() }).where("status", "=", "connected").execute();
  }

  async messages(chatId: string) {
    const rows = await this.database.selectFrom("codelogicx_telegram_messages").selectAll().where("chat_id", "=", chatId).orderBy("created_at", "asc").limit(200).execute();
  ...
  async addMessage(chatId: string, direction: "inbound" | "outbound", body: string, telegramMessageId?: string) {
    await this.database.insertInto("codelogicx_telegram_messages").values({

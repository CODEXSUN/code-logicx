...
export async function registerIdeasRoutes(app: FastifyInstance) {
  ...
  app.get("/ideas/users", async (request) =>
    ok(await requireCodeLogicXUserDirectory().list(), { requestId: request.id })
  );
...
function actor() {
  const value = requireCodeLogicXActor();
...
async function validateAssignees(assigneeUuids: string[] | undefined) {
  ...
  const activeUsers = await requireCodeLogicXUserDirectory().list();

...
export class IdeasRepository {
  ...
  async update(
    ...
    if (Object.keys(values).length)
      await this.db.updateTable("codelogicx_ideas").set(values).where("uuid", "=", uuid).execute();
    return this.find(uuid, actor);
  }
  ...
  async remove(uuid: string, actor: string) {
    ...
    const storedImages = await this.db
      .selectFrom("codelogicx_idea_attachments")
      .select("storage_key")
    ...
    await this.db.transaction().execute(async (transaction) => {
      const comments = await transaction
        .selectFrom("codelogicx_idea_comments")
        .select("uuid")
        .where("idea_uuid", "=", uuid)
        .execute();
      const poll = await transaction
        .selectFrom("codelogicx_idea_polls")
        .select("uuid")
        .where("idea_uuid", "=", uuid)
        .executeTakeFirst();
      if (comments.length)
        await transaction
          .deleteFrom("codelogicx_idea_likes")
      ...
      if (poll)
        await transaction
          .deleteFrom("codelogicx_idea_poll_votes")
      ...
      await transaction
        ...
        .execute();
      await transaction
        .deleteFrom("codelogicx_idea_comments")
        .where("idea_uuid", "=", uuid)
        .execute();
      await transaction
        .deleteFrom("codelogicx_idea_attachments")
        .where("idea_uuid", "=", uuid)
        .execute();
      await transaction
        .deleteFrom("codelogicx_idea_drawings")
        .where("idea_uuid", "=", uuid)
        .execute();
      await transaction.deleteFrom("codelogicx_idea_polls").where("idea_uuid", "=", uuid).execute();
      await transaction.deleteFrom("codelogicx_ideas").where("uuid", "=", uuid).execute();
    });
  ...
  async addComment(ideaUuid: string, bodyHtml: string, parentUuid: string | null, actor: string) {
    ...
    const uuid = uid();
    await this.db
      .insertInto("codelogicx_idea_comments")
      .values({
        uuid,
        idea_uuid: ideaUuid,
  ...
  async toggleLike(kind: "idea" | "comment", entityUuid: string, actor: string) {
    ...
    const existing = await this.db
      .selectFrom("codelogicx_idea_likes")
      .select("uuid")
      ...
      .executeTakeFirst();
    if (existing)
      await this.db.deleteFrom("codelogicx_idea_likes").where("uuid", "=", existing.uuid).execute();
    else
      await this.db
        .insertInto("codelogicx_idea_likes")
  ...
  async savePoll(ideaUuid: string, input: PollInput, actor: string) {
    ...
    if (existing)
      await this.db
        .updateTable("codelogicx_idea_polls")
        .set({
      ...
      await this.db
        .insertInto("codelogicx_idea_polls")
        .values({
          uuid: uid(),
          idea_uuid: ideaUuid,
          question: input.question,
  ...
  async vote(ideaUuid: string, optionId: string, actor: string) {
    ...
    if (!poll.multiple_choice)
      await this.db
        ...
        .execute();
    await this.db
      .insertInto("codelogicx_idea_poll_votes")
      .ignore()
  ...
  async addAttachment(
    ...
    try {
      await this.db
        .insertInto("codelogicx_idea_attachments")
        .values({
          uuid,
          idea_uuid: ideaUuid,
          name: image.name,
  ...
  async saveDrawing(ideaUuid: string, scene: unknown, actor: string) {
    await this.find(ideaUuid, actor);
    const existing = await this.db
      .selectFrom("codelogicx_idea_drawings")
      .select("uuid")
      .where("idea_uuid", "=", ideaUuid)
      .executeTakeFirst();
    if (existing)
      await this.db
        .updateTable("codelogicx_idea_drawings")
        .set({ scene_json: JSON.stringify(scene), updated_by: actor })
        .where("uuid", "=", existing.uuid)
        .execute();
    else
      await this.db
        .insertInto("codelogicx_idea_drawings")
        .values({
          uuid: uid(),
          idea_uuid: ideaUuid,
  ...
  private mapAttachment(item: {
    ...
    const imageUrl = `/api/codelogicx/ideas/${item.idea_uuid}/attachments/${item.uuid}/image?access=${ideaImageAccessToken(item.idea_uuid, item.uuid)}`;
    return {
      uuid: item.uuid,
  ...
  private async poll(ideaUuid: string) {
    ...
    const votes = await this.db
      .selectFrom("codelogicx_idea_poll_votes")

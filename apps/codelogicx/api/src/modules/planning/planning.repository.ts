...
export class PlanningRepository {
  ...
  async list(record?: { kind: PlanningRecordKind; uuid: string }) {
    let query = this.database
      .selectFrom("codelogicx_planning_boards")
      .selectAll()
      .where("sync_status", "!=", "deleted");
    if (record)
      query = query.where("uuid", "in", (builder) =>
        builder
          .selectFrom("codelogicx_planning_board_links")
          .select("board_uuid")
  ...
  async create(
    ...
    await this.database
      .insertInto("codelogicx_planning_boards")
      .values({
        created_by: actor,
  ...
  async link(boardUuid: string, kind: PlanningRecordKind, recordUuid: string, actor: string) {
    ...
    await this.database
      .insertInto("codelogicx_planning_board_links")
      .values({
  ...
  async comments(boardUuid: string) {
    ...
    const rows = await this.database
      .selectFrom("codelogicx_planning_comments")
      .selectAll()
    ...
    const reactions = await this.database
      .selectFrom("codelogicx_planning_reactions")
      .selectAll()
  ...
  async createComment(
    ...
    await this.database
      .insertInto("codelogicx_planning_comments")
      .values({
        board_uuid: boardUuid,
        body: input.body,
        created_by: actor,
  ...
  async setCommentResolved(commentUuid: string, resolved: boolean, actor: string) {
    ...
    await this.database
      .updateTable("codelogicx_planning_comments")
      .set({
  ...
  async toggleReaction(commentUuid: string, reaction: string, actor: string) {
    ...
    if (existing)
      await this.database
        .updateTable("codelogicx_planning_reactions")
        .set({
      ...
      await this.database
        .insertInto("codelogicx_planning_reactions")
        .values({
  ...
  async update(
    ...
    await this.database
      .updateTable("codelogicx_planning_boards")
      .set({
  ...
  async delete(uuid: string, actor: string) {
    ...
    await this.database
      ...
      .executeTakeFirst();
    await this.database
      .updateTable("codelogicx_planning_board_links")
      .set({
        sync_status: "deleted",
        sync_updated_at: new Date(),
        sync_version: (eb) => eb("sync_version", "+", 1)
  ...
  private async requireRecord(kind: PlanningRecordKind, recordUuid: string) {
    const record = await this.database
      ...
      .where("sync_status", "!=", "deleted")
      .executeTakeFirst();
    if (!record) throw AppError.validation(`Selected CodeLogicX ${kind} was not found.`);
  }

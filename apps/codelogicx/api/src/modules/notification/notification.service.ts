...
export class NotificationService {
  private readonly repository = new NotificationRepository(getCodeLogicXDatabase());

  async create(input: CreateNotificationInput) {
    const result = await this.repository.create(requireCodeLogicXActor().id, input);
    await enqueueNotificationJobs(result.jobIds);
    return toNotification(result.notification);
  }

  async inbox() {
    const rows = await this.repository.list(requireCodeLogicXActor().id);
    return rows.map(toNotification);
  }

  async markRead(uuid: string) {
    return toNotification(await this.repository.markRead(uuid, requireCodeLogicXActor().id));
  }

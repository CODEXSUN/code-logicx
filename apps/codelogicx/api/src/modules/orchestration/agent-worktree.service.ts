...
export class AgentWorktreeService {
  ...
  async prepareChild(input: { access: AgentAccessMode; runId: string; sourceRoot: string }) {
    ...
    if (!allowedRoots().some((allowed) => contains(allowed, sourceRoot))) {
      throw new AppError({ code: "AGENT_REPOSITORY_NOT_ALLOWED", message: "The parent repository is outside CODELOGICX_AGENT_ALLOWED_ROOTS.", statusCode: 403 });
    }
    const baseRevision = await git(sourceRoot, ["rev-parse", "HEAD"]);
    if (!needsWorktree(input.access)) {
  ...
  private async resolveSourceRoot(referenceId: string, referenceType: string) {
    ...
    if (!allowedRoots().some((allowed) => contains(allowed, root))) {
      throw new AppError({
        ...
        message: "The project repository is outside CODELOGICX_AGENT_ALLOWED_ROOTS.",
        statusCode: 403
      });
    }
    return root;
...
function allowedRoots() {
  const configured = process.env.CODELOGICX_AGENT_ALLOWED_ROOTS?.trim();
  return (configured ? configured.split(delimiter) : [process.cwd()]).filter(Boolean).map((path) => resolve(path));
}

function worktreeRoot() {
  const configured = process.env.CODELOGICX_AGENT_WORKTREE_ROOT?.trim();
  if (configured) return resolve(configured);
  const applicationData = process.env.LOCALAPPDATA?.trim();
  if (applicationData) return resolve(applicationData, "CodeLogicX", "CodeLogicX", "worktrees");
  return resolve(process.env.CODELOGICX_STORAGE_PATH?.trim() || join(process.cwd(), "storage", "codelogicx"), "agent-worktrees");
}

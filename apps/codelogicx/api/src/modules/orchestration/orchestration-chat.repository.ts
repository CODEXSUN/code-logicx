...
export class OrchestrationChatRepository {
  ...
  async list(actorId: string) {
    const rows = await this.database
      .selectFrom("codelogicx_orchestration_chat_threads")
      .selectAll()
  ...
  async find(uuid: string, actorId: string) {
    ...
    const messages = await this.database
      .selectFrom("codelogicx_orchestration_chat_messages")
      .selectAll()
  ...
  async create(
    ...
    await this.database
      .insertInto("codelogicx_orchestration_chat_threads")
      .values({
  ...
  async updateRuntime(
    ...
    await this.database
      .updateTable("codelogicx_orchestration_chat_threads")
      .set({
        access_mode: input.access,
  ...
  async addMessage(
    ...
    const uuid = randomBytes(8).toString("hex");
    await this.database
      .insertInto("codelogicx_orchestration_chat_messages")
      .values({
  ...
  async setFeedback(messageUuid: string, feedback: "down" | "up" | null, actorId: string) {
    const result = await this.database
      .updateTable("codelogicx_orchestration_chat_messages")

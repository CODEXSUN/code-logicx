...
export class HostingerMcpService {
  ...
  private writeManagedConfiguration() {
    const path = configPath();
    mkdirSync(resolveCodeLogicXCodexHome(), { recursive: true });
...
function configPath() {
  return `${resolveCodeLogicXCodexHome()}/config.toml`;
}

function environmentPath() {
  const configured = process.env.CODELOGICX_ENV_FILE_PATH?.trim();

...
export class AgentTaskGraphRepository {
  private readonly database = getCodeLogicXDatabase();
  ...
  async replace(parentRunUuid: string, actorId: string, input: AgentDecompositionInput) {
    ...
    await this.database.transaction().execute(async (transaction) => {
      ...
      if (dependencies.length) {
        await transaction
          .insertInto("codelogicx_agent_task_dependencies")
          .values(dependencies)
          .execute();
      }
      await transaction
        .updateTable("codelogicx_agent_runs")
        .set({ review_status: "tasks_pending" })
        .where("uuid", "=", parentRunUuid)
        .where("actor_id", "=", actorId)
        .execute();
      await transaction
        .insertInto("codelogicx_agent_events")
        .values({
          actor_id: actorId,
  ...
  async start(taskUuid: string, actorId: string) {
    ...
    const running = await this.database
      .selectFrom("codelogicx_agent_tasks")
    ...
    await this.database
      .updateTable("codelogicx_agent_tasks")
      .set({
  ...
  async executionContext(taskUuid: string, actorId: string) {
    ...
    const [child, parent, persona, dependencies] = await Promise.all([
      this.database
        .selectFrom("codelogicx_agent_runs")
        .selectAll()
      ...
      this.database
        .selectFrom("codelogicx_agent_task_dependencies as dependency")
        .innerJoin(
          "codelogicx_agent_tasks as required",
          "required.uuid",
          "dependency.depends_on_task_uuid"
        )
        .leftJoin("codelogicx_agent_runs as child", "child.uuid", "required.child_run_uuid")
  ...
  async review(
    ...
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .insertInto("codelogicx_agent_parent_reviews")
        .values({
          actor_id: actorId,
          decision,
          note,
  ...
  async find(parentRunUuid: string, actorId: string) {
    ...
    const [parent, tasks, dependencies, reviews, personas, approvals] = await Promise.all([
      ...
      this.database
        .selectFrom("codelogicx_agent_task_dependencies as dependency")
        .innerJoin("codelogicx_agent_tasks as task", "task.uuid", "dependency.task_uuid")
        .select(["dependency.task_uuid", "dependency.depends_on_task_uuid"])
        .where("task.parent_run_uuid", "=", parentRunUuid)
        .execute(),
      this.database
        .selectFrom("codelogicx_agent_parent_reviews")
        .selectAll()
        .where("parent_run_uuid", "=", parentRunUuid)
        .orderBy("created_at", "desc")
        .execute(),
      this.database
        .selectFrom("codelogicx_agent_personas")
        .selectAll()
        .where("actor_id", "=", actorId)
        .where("status", "=", "active")
        .execute(),
      this.database
        .selectFrom("codelogicx_agent_approvals as approval")
        .innerJoin("codelogicx_agent_tasks as task", "task.child_run_uuid", "approval.run_uuid")
        .select([
          "approval.reason",
          "approval.request_id",
          "approval.status",
...
async function refreshReadyTasks(transaction: Transaction<CodeLogicXDatabase>, parentRunUuid: string) {

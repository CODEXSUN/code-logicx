...
export class GitWorktreeIntegrator {
  ...
  async commit(path: string, message: string) {
    ...
    await git(path, [
      "-c", `user.name=${process.env.CODELOGICX_AGENT_GIT_NAME?.trim() || "CodeLogicX CodeLogicX"}`,
      "-c", `user.email=${process.env.CODELOGICX_AGENT_GIT_EMAIL?.trim() || "codelogicx@localhost"}`,
      "commit", "-m", message
    ], 120_000);

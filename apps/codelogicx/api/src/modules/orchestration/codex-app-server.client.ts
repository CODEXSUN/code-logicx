...
export class CodexAppServerClient {
  ...
  async startThread(cwd: string, model: string, access: CodexAccess) {
    ...
    const result = (await this.request("thread/start", {
      ...
      serviceName: "codelogicx_codelogicx"
    })) as { thread: { id: string } };
  ...
  private async start() {
    ...
    await this.request("initialize", {
      clientInfo: { name: "codelogicx_codelogicx", title: "CodeLogicX CodeLogicX", version: "1.0.22" }
    });
...
export function resolveCodeLogicXCodexHome(connectionId: string) {
  const configured = process.env.CODELOGICX_CODEX_HOME?.trim();
  if (configured) {
  ...
  const base = join(applicationData, "CodeLogicX", "CodeLogicX", "codex");

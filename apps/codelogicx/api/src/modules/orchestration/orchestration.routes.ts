...
export async function registerOrchestrationRoutes(app: FastifyInstance) {
  ...
  app.get("/orchestration/model-providers", async (request) =>
    ok(await modelProviderService.list(requireCodeLogicXActor().id), { requestId: request.id })
  );
  ...
  app.post("/orchestration/model-providers/:provider/test", async (request) => {
    const { provider } = modelProviderParamSchema.parse(request.params);
    return ok(await modelProviderService.test(provider, requireCodeLogicXActor().id), {
      requestId: request.id
    });
  });
  app.post("/orchestration/model-providers/:provider/disconnect", async (request) => {
    const { provider } = modelProviderParamSchema.parse(request.params);
    return ok(await modelProviderService.remove(provider, requireCodeLogicXActor().id), {
      requestId: request.id
    });
  });
  ...
  app.post("/orchestration/agent-ide/codex/chat/stream", async (request, reply) => {
    ...
    const actorId = requireCodeLogicXActor().id;
    reply.hijack();
  ...
  app.get("/orchestration/agent-ide/chats", async (request) =>
    ok(await orchestrationChatRepository.list(requireCodeLogicXActor().id), { requestId: request.id })
  );
  app.get("/orchestration/agent-ide/runs", async (request) => {
    const { projectUuid } = z
      ...
      .strict()
      .parse(request.query);
    return ok(await agentRunRepository.list(projectUuid, requireCodeLogicXActor().id), {
      requestId: request.id
    });
  });
  app.get("/orchestration/agent-ide/personas", async (request) =>
    ok(await agentPersonaRepository.list(requireCodeLogicXActor().id), { requestId: request.id })
  );
  ...
  app.post("/orchestration/agent-ide/personas/starter-team", async (request) =>
    ok(await agentPersonaRepository.createStarterTeam(requireCodeLogicXActor().id), {
      requestId: request.id
    })
  );
  app.get("/orchestration/agent-ide/runs/:uuid", async (request) => {
    const { uuid } = z
      ...
      .strict()
      .parse(request.params);
    return ok(await agentRunRepository.find(uuid, requireCodeLogicXActor().id), {
      requestId: request.id
    });
  });
  app.get("/orchestration/agent-ide/runs/:uuid/tasks", async (request) => {
    const { uuid } = z
      ...
      .strict()
      .parse(request.params);
    return ok(await agentTaskGraphRepository.find(uuid, requireCodeLogicXActor().id), {
      requestId: request.id
    });
  });
  app.put("/orchestration/agent-ide/runs/:uuid/tasks", async (request) => {
    ...
    return ok(await agentTaskGraphRepository.replace(uuid, requireCodeLogicXActor().id, input), {
      requestId: request.id
    });
  });
  app.post("/orchestration/agent-ide/tasks/:uuid/start", async (request) => {
    const { uuid } = z
      ...
      .strict()
      .parse(request.params);
    return ok(await agentDelegateExecutor.call(uuid, requireCodeLogicXActor().id), {
      requestId: request.id
    });
  });
  app.put("/orchestration/agent-ide/tasks/:uuid/delegate", async (request) => {
    ...
    return ok(
      await agentTaskGraphRepository.assignDelegate(uuid, requireCodeLogicXActor().id, personaUuid),
      { requestId: request.id }
    );
  });
  app.put("/orchestration/agent-ide/runs/:uuid/supervisor", async (request) => {
    ...
    const { personaUuid } = agentPersonaAssignmentSchema.parse(request.body);
    return ok(
      await agentTaskGraphRepository.assignSupervisor(uuid, requireCodeLogicXActor().id, personaUuid),
      { requestId: request.id }
    );
  });
  ...
  app.post("/orchestration/agent-ide/runs/:uuid/verification", async (request) => {
    const { uuid } = z
      ...
      .strict()
      .parse(request.params);
    return ok(await agentVerificationService.run(uuid, requireCodeLogicXActor().id), {
      requestId: request.id
    });
  });
  app.post("/orchestration/agent-ide/runs/:uuid/rework", async (request) => {
    ...
    return ok(await agentRunRepository.requestRework(uuid, requireCodeLogicXActor().id, note), {
      requestId: request.id
    });
  });
  app.post("/orchestration/agent-ide/runs/:uuid/commit", async (request) => {
    ...
    return ok(await agentIntegrationService.commit(uuid, requireCodeLogicXActor().id, message), {
      requestId: request.id
    });
  });
  ...
  app.get("/orchestration/agent-ide/chats/:uuid", async (request) => {
    const { uuid } = z
      ...
      .strict()
      .parse(request.params);
    return ok(await orchestrationChatRepository.find(uuid, requireCodeLogicXActor().id), {
      requestId: request.id
    });
  });
  app.delete("/orchestration/agent-ide/chats/:uuid", async (request) => {
    const { uuid } = z
      ...
      .strict()
      .parse(request.params);
    return ok(await orchestrationChatRepository.archive(uuid, requireCodeLogicXActor().id), {
      requestId: request.id
    });
  });
  app.put("/orchestration/agent-ide/chat-messages/:uuid/feedback", async (request) => {
    ...
    const { feedback } = z
      ...
      .parse(request.body);
    return ok(
      await orchestrationChatRepository.setFeedback(uuid, feedback, requireCodeLogicXActor().id),
      { requestId: request.id }
    );
  });
  app.post("/orchestration/agent-ide/codex/approval", async (request) => {
    ...
    await agentRunRepository.resolveApproval(requireCodeLogicXActor().id, input);

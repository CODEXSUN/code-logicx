  ...
  async test(target: HostingerSshTarget) {
    ...
    try {
      const { stdout } = await execFileAsync("ssh", [
        ...
        "printf 'connected=true\\nhost='; hostname; printf 'user='; id -un; test -d /home/codelogicx && printf 'codelogicxPath=present\\n' || printf 'codelogicxPath=missing\\n'"
...
function keyPaths(virtualMachineId: number) {
  const directory = resolve(process.env.CODELOGICX_STORAGE_PATH?.trim() || join(process.cwd(), "storage", "codelogicx"), "hostinger-ssh");

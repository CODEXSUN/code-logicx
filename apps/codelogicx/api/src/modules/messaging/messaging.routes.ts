...
import { z } from "zod";
import { requireCodeLogicXActor, requireCodeLogicXUserDirectory } from "../../request-context.js";
...
export async function registerMessagingRoutes(app: FastifyInstance) {
  app.get("/messaging/contacts", async (request) => {
    ...
    const actor = requireCodeLogicXActor();
    const search = query.search?.trim().toLocaleLowerCase() ?? "";
    const users = await requireCodeLogicXUserDirectory().list();
  ...
  app.get("/messaging/conversations", async (request) =>
    ok(await repository.list(requireCodeLogicXActor().id), { requestId: request.id }));
  ...
  app.get("/messaging/conversations/:id/messages", async (request) => {
    ...
    return ok(await repository.messages(id, requireCodeLogicXActor().id), { requestId: request.id });
  });
  app.post("/messaging/conversations/:id/messages", { bodyLimit: 15 * 1024 * 1024 }, async (request) => {
    ...
    const validUsers = new Set((await requireCodeLogicXUserDirectory().list()).map((user) => user.uuid));
...
async function actorContact(): Promise<MessagingContact> {
  const actor = requireCodeLogicXActor();
  const user = (await requireCodeLogicXUserDirectory().list()).find((candidate) => candidate.uuid === actor.id);

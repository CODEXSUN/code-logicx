...
export class MessagingRepository {
  ...
  async list(actorId: string) {
    const conversations = await sql<ConversationRow>`SELECT c.* FROM codelogicx_messaging_conversations c
      INNER JOIN codelogicx_messaging_members m ON m.conversation_uuid = c.uuid
  ...
  async find(id: string, actorId: string) {
    const rows = await sql<ConversationRow>`SELECT c.* FROM codelogicx_messaging_conversations c
      INNER JOIN codelogicx_messaging_members m ON m.conversation_uuid = c.uuid
  ...
  async findDirect(actorId: string, otherId: string) {
    const rows = await sql<ConversationRow>`SELECT c.* FROM codelogicx_messaging_conversations c
      INNER JOIN codelogicx_messaging_members mine ON mine.conversation_uuid = c.uuid AND mine.user_uuid = ${actorId}
      INNER JOIN codelogicx_messaging_members other ON other.conversation_uuid = c.uuid AND other.user_uuid = ${otherId}
      WHERE c.type = 'direct' AND (SELECT COUNT(*) FROM codelogicx_messaging_members x WHERE x.conversation_uuid = c.uuid) = 2
  ...
  async create(type: "direct" | "group", title: string, creator: MessagingContact, members: MessagingContact[]) {
    ...
    await this.database.transaction().execute(async (trx) => {
      await sql`INSERT INTO codelogicx_messaging_conversations (uuid, type, title, created_by_uuid)
        VALUES (${id}, ${type}, ${title}, ${creator.uuid})`.execute(trx);
      for (const member of members) {
        await sql`INSERT INTO codelogicx_messaging_members
  ...
  async messages(conversationId: string, actorId: string) {
    ...
    const rows = await sql<MessageRow>`SELECT * FROM codelogicx_messaging_messages
      WHERE conversation_uuid = ${conversationId} ORDER BY sequence_number ASC LIMIT 500`.execute(this.database);
    const members = await sql<MemberRow>`SELECT * FROM codelogicx_messaging_members
  ...
  async send(conversationId: string, actor: MessagingContact, content: string, clientMessageId: string, mentionIds: string[], attachment: MessagingAttachment | null) {
    ...
    const existing = await sql<MessageRow>`SELECT * FROM codelogicx_messaging_messages
    ...
    const row = await this.database.transaction().execute(async (trx) => {
      await sql`UPDATE codelogicx_messaging_conversations SET last_sequence = last_sequence + 1
        WHERE uuid = ${conversationId}`.execute(trx);
      const sequence = await sql<{ last_sequence: number }>`SELECT last_sequence FROM codelogicx_messaging_conversations
        WHERE uuid = ${conversationId}`.execute(trx);
      const id = randomBytes(4).toString("hex");
      await sql`INSERT INTO codelogicx_messaging_messages
      ...
      await sql`UPDATE codelogicx_messaging_conversations SET updated_at = CURRENT_TIMESTAMP WHERE uuid = ${conversationId}`.execute(trx);
      const saved = await sql<MessageRow>`SELECT * FROM codelogicx_messaging_messages WHERE uuid = ${id}`.execute(trx);
      return saved.rows[0];
    });
  ...
  async react(conversationId: string, messageId: string, actor: MessagingContact, emoji: string | null) {
    ...
    if (emoji) {
      await sql`INSERT INTO codelogicx_messaging_reactions (message_uuid, user_uuid, user_name, emoji)
      ...
      await sql`DELETE FROM codelogicx_messaging_reactions WHERE message_uuid = ${messageId} AND user_uuid = ${actor.uuid}`.execute(this.database);
    }
    const members = await sql<MemberRow>`SELECT * FROM codelogicx_messaging_members WHERE conversation_uuid = ${conversationId}`.execute(this.database);
  ...
  async markRead(conversationId: string, actorId: string, sequence: number) {
    await this.requireMember(conversationId, actorId);
    const result = await sql`UPDATE codelogicx_messaging_members SET last_read_sequence = ${sequence}
  ...
  async memberIds(conversationId: string) {
    const rows = await sql<{ user_uuid: string }>`SELECT user_uuid FROM codelogicx_messaging_members
      WHERE conversation_uuid = ${conversationId}`.execute(this.database);
    return rows.rows.map((row) => row.user_uuid);
  }

  private async requireMember(conversationId: string, actorId: string) {
    const rows = await sql<{ count: number | string }>`SELECT COUNT(*) count FROM codelogicx_messaging_members
  ...
  private async mapConversation(row: ConversationRow, actorId: string): Promise<MessagingConversation> {
    const members = await sql<MemberRow>`SELECT * FROM codelogicx_messaging_members WHERE conversation_uuid = ${row.uuid}`.execute(this.database);
    const last = await sql<MessageRow>`SELECT * FROM codelogicx_messaging_messages WHERE conversation_uuid = ${row.uuid}
  ...
  private async reactions(messageIds: string[]) {
    ...
    const rows = await sql<ReactionRow>`SELECT message_uuid, user_uuid, user_name, emoji
      FROM codelogicx_messaging_reactions WHERE message_uuid IN (${sql.join(messageIds)})`.execute(this.database);
    return rows.rows;
  }
}

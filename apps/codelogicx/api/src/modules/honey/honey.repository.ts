...
class HoneyRepository {
  async list(actorId: string) {
    return getCodeLogicXDatabase()
      .selectFrom("codelogicx_honey_threads")
      .selectAll()
  ...
  async find(threadUuid: string, actorId: string) {
    const thread = await getCodeLogicXDatabase()
      .selectFrom("codelogicx_honey_threads")
      .selectAll()
  ...
  async create(actorId: string, firstMessage: string) {
    ...
    await getCodeLogicXDatabase()
      .insertInto("codelogicx_honey_threads")
      .values({
        actor_id: actorId,
  ...
  async messages(threadUuid: string, actorId: string) {
    ...
    return getCodeLogicXDatabase()
      .selectFrom("codelogicx_honey_messages")
      .selectAll()
  ...
  async addMessage(
    ...
    await getCodeLogicXDatabase()
      .insertInto("codelogicx_honey_messages")
      .values({
        actor_id: actorId,
        body,
      ...
      .executeTakeFirstOrThrow();
    await getCodeLogicXDatabase()
      .updateTable("codelogicx_honey_threads")
  ...
  async archive(threadUuid: string, actorId: string) {
    const result = await getCodeLogicXDatabase()
      .updateTable("codelogicx_honey_threads")
  ...
  async approvedMemory(actorId: string) {
    return getCodeLogicXDatabase()
      .selectFrom("codelogicx_honey_memory")
      .select(["kind", "content"])
      .where("actor_id", "=", actorId)
  ...
  async reviewMemory(
    ...
    const result = await getCodeLogicXDatabase()
      .updateTable("codelogicx_honey_memory")
  ...
  async rememberCandidate(actorId: string, threadUuid: string, content: string) {
    ...
    if (existing) return;
    await getCodeLogicXDatabase()
      .insertInto("codelogicx_honey_memory")
      .values({
        actor_id: actorId,
        content,

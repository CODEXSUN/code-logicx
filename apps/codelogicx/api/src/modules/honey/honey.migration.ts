...
export const honeyMigration = {
  ...
  key: "codelogicx.honey.sql.v3"
} as const;

export async function migrateHoneyModule(database: Kysely<CodeLogicXDatabase>) {
  await sql`CREATE TABLE IF NOT EXISTS codelogicx_honey_threads (
    ...
    UNIQUE KEY uq_codelogicx_honey_threads_uuid (uuid),
    KEY idx_codelogicx_honey_threads_actor (actor_id, updated_at)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`.execute(database);
  await sql`CREATE TABLE IF NOT EXISTS codelogicx_honey_messages (
    id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, uuid CHAR(16) NOT NULL,
    thread_uuid CHAR(16) NOT NULL, actor_id VARCHAR(160) NOT NULL,
    role VARCHAR(16) NOT NULL, body TEXT NOT NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uq_codelogicx_honey_messages_uuid (uuid),
    KEY idx_codelogicx_honey_messages_thread (thread_uuid, created_at),
    KEY idx_codelogicx_honey_messages_actor (actor_id, created_at)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`.execute(database);
  await sql`CREATE TABLE IF NOT EXISTS codelogicx_honey_memory (
    id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, uuid CHAR(16) NOT NULL,
    ...
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY uq_codelogicx_honey_memory_uuid (uuid),
    KEY idx_codelogicx_honey_memory_actor (actor_id, status, updated_at)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`.execute(database);
  await sql`ALTER TABLE codelogicx_honey_messages
    ADD COLUMN IF NOT EXISTS context_json TEXT NOT NULL DEFAULT ('{}') AFTER body`.execute(database);
  await sql`ALTER TABLE codelogicx_honey_memory

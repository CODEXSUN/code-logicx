...
const actions: Record<HoneyAction["id"], HoneyAction> = {
  ...
  "open-project": { href: "/app/codelogicx/projects", id: "open-project", label: "Open project" },
  "review-deployment": {
    href: "/app/codelogicx/orchestration",
    id: "review-deployment",
    label: "Review deployment"
  },
  "start-agent": { href: "/app/codelogicx/agent-ide", id: "start-agent", label: "Start Project Agent" },
  "view-task": { href: "/app/codelogicx/tasks", id: "view-task", label: "View task" }
};
...
function contextualAction(
  ...
  if (id === "open-project" && context?.projectId) {
    return {
      ...actions[id],
      href: `/app/codelogicx/projects?project=${encodeURIComponent(context.projectId)}`
    };
  }
  if (id === "view-task" && context?.taskId) {
    return { ...actions[id], href: `/app/codelogicx/tasks?task=${encodeURIComponent(context.taskId)}` };
  }
  ...
  if (id === "start-agent") {
    ...
    return {
      ...actions[id],
      href: `/app/codelogicx/agent-ide?${params.toString()}`,

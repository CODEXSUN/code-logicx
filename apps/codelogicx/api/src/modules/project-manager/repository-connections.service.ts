...
import { getCodeLogicXDatabase } from "../../database/index.js";
...
export class RepositoryConnectionsService {
  async listForSettings() {
    const rows = await getCodeLogicXDatabase()
      .selectFrom("codelogicx_repository_connections")
      .selectAll()
      .orderBy("name", "asc")
      .execute();
  ...
  async create(input: RepositoryConnectionInput) {
    await getCodeLogicXDatabase()
      .insertInto("codelogicx_repository_connections")
      .values({
  ...
  async update(id: string, input: RepositoryConnectionInput) {
    const result = await getCodeLogicXDatabase()
      .updateTable("codelogicx_repository_connections")
      .set({
        base_url: normalizeBaseUrl(input.baseUrl),
        name: input.name.trim(),
        provider: input.provider,

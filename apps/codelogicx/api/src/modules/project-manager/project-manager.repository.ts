...
export class ProjectManagerRepository {
  ...
  async create(record: ProjectManagerRecord, actorEmail: string) {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .insertInto("codelogicx_project_manager_items")
    ...
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        ...
        .set({
          ...
          dependencies_json: JSON.stringify(record.dependencyIds),
  ...
  async listAttachments(recordKind: ProjectManagerAttachmentKind, recordUuid: string) {
    const rows = await this.database
      .selectFrom("codelogicx_project_manager_attachments")
      .selectAll()
  ...
  async createAttachment(input: ProjectManagerAttachmentCreate, actorEmail: string) {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .insertInto("codelogicx_project_manager_attachments")
        .values({
  ...
  async deleteAttachment(attachment: ProjectManagerAttachment, actorEmail: string) {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .updateTable("codelogicx_project_manager_attachments")
        .set({
          sync_direction: "outbound",
          sync_status: "deleted",
          sync_updated_at: new Date()
  ...
  async listRegistryPlatforms() {
    const rows = await this.database
      .selectFrom("codelogicx_project_manager_registry_platforms")
      .selectAll()
      .orderBy("sort_order")
  ...
  async createRegistryPlatform(record: ProjectManagerRegistryPlatform, actorEmail: string) {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .insertInto("codelogicx_project_manager_registry_platforms")
  ...
  async updateRegistryPlatform(record: ProjectManagerRegistryPlatform, actorEmail: string) {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .updateTable("codelogicx_project_manager_registry_platforms")
        .set({
          active: record.active ? 1 : 0,
          description: record.description,
          name: record.name,
  ...
  async listRegistryGroups() {
    const rows = await this.database
      .selectFrom("codelogicx_project_manager_registry_groups")
      .selectAll()
      .orderBy("sort_order")
      .orderBy("updated_at", "desc")
      .execute();
  ...
  async createRegistryGroup(record: ProjectManagerRegistryGroup, actorEmail: string) {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .insertInto("codelogicx_project_manager_registry_groups")
  ...
  async updateRegistryGroup(record: ProjectManagerRegistryGroup, actorEmail: string) {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .updateTable("codelogicx_project_manager_registry_groups")
        .set({
          active: record.active ? 1 : 0,
          description: record.description,
  ...
  async listRegistryModules() {
    const rows = await this.database
      .selectFrom("codelogicx_project_manager_registry_modules")
      .selectAll()
      .orderBy("sort_order")
      .orderBy("updated_at", "desc")
      .execute();
  ...
  async createRegistryModule(record: ProjectManagerRegistryModule, actorEmail: string) {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .insertInto("codelogicx_project_manager_registry_modules")
  ...
  async updateRegistryModule(record: ProjectManagerRegistryModule, actorEmail: string) {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .updateTable("codelogicx_project_manager_registry_modules")
        .set({
          active: record.active ? 1 : 0,
          description: record.description,
...
async function writeActivity(database: Kysely<CodeLogicXDatabase>, input: AuditInput) {
  await database
    .insertInto("codelogicx_project_manager_activity")
    .values({
      action: input.action,
...
function mapItem(row: Selectable<ProjectManagerItemsTable>): ProjectManagerRecord {
  return {
    ...
    dependencyIds: parseJson(row.dependencies_json, []),

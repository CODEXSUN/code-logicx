...
import { RepositoryConnectionsService } from "./repository-connections.service.js";

const service = new ProjectManagerService();
const repositoryConnections = new RepositoryConnectionsService();
...
const repositoryConnectionSchema = z
  .object({
    ...
    provider: z.enum(["github", "private-git"]),
    repositorySlug: z.string().trim().min(1).max(240),
...
export async function registerProjectManagerRoutes(app: FastifyInstance) {
  ...
  app.get("/project-manager/repositories", async (request) =>
    ok(await repositoryConnections.listForDevelopers(), { requestId: request.id })
  );
  app.post("/admin/repository-connections", async (request) =>
    ok(await repositoryConnections.create(repositoryConnectionSchema.parse(request.body)), {
      requestId: request.id
    })
  );
  app.put("/admin/repository-connections/:id", async (request) =>
    ok(
      await repositoryConnections.update(
        idParamsSchema.parse(request.params).id,
        repositoryConnectionSchema.parse(request.body)
      ),
      { requestId: request.id }
    )
  );
  app.post("/project-manager/select-local-folder", async (request) =>
    ok(await repositoryConnections.selectLocalFolder(), { requestId: request.id })
  );
...
function actor(request: FastifyRequest) {
  return requireCodeLogicXActor().email?.trim() || requireCodeLogicXActor().id || request.id;
}

...
export class TaskManagerRepository {
  ...
  async update(scopeKey: string, record: Todo, actorEmail: string, action = "updated") {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .updateTable("codelogicx_task_manager_todos")
        .set({
  ...
  async listLookups(scopeKey: string) {
    const rows = await this.database
      .selectFrom("codelogicx_task_manager_lookups")
      .selectAll()
      .where("scope_key", "=", scopeKey)
      .where("sync_status", "!=", "deleted")
      .orderBy("kind")
  ...
  async createLookup(scopeKey: string, record: TodoLookup, actorEmail: string) {
    await this.database.transaction().execute(async (transaction) => {
      await transaction
        .insertInto("codelogicx_task_manager_lookups")
        .values({
...
async function writeActivity(
  ...
  await database
    .insertInto("codelogicx_task_manager_activity")
    .values({
      action,

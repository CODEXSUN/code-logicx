...
import { CodeLogicXSyncService } from "./sync.service.js";

const service = new CodeLogicXSyncService();
...
function syncToken(request: FastifyRequest) {
  return tokenSchema.parse(request.headers["x-codelogicx-sync-token"]);
}

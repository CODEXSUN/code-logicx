...
export function syncTokenHash(token: string) {
  return createHash("sha256")
    .update(`${token}:${requiredSecret("CODELOGICX_SYNC_TOKEN_PEPPER")}`)
    .digest("hex");
}
...
export function decryptSyncToken(value: string) {
  ...
  if (version !== "v1" || !ivValue || !tagValue || !ciphertextValue) {
    throw new Error("Stored CodeLogicX sync token is invalid.");
  }
...
function encryptionKey() {
  return createHash("sha256").update(requiredSecret("CODELOGICX_SYNC_ENCRYPTION_KEY")).digest();
}

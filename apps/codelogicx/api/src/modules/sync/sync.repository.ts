...
import type { CodeLogicXSyncSnapshot } from "./sync.types.js";

export const synchronizedTables = [
  "codelogicx_planning_boards",
  "codelogicx_planning_board_links",
  "codelogicx_planning_comments",
  "codelogicx_planning_reactions",
  "codelogicx_project_manager_registry_platforms",
  "codelogicx_project_manager_registry_groups",
  "codelogicx_project_manager_registry_modules",
  "codelogicx_project_manager_items",
  "codelogicx_project_manager_activity",
  "codelogicx_project_manager_attachments",
  "codelogicx_task_manager_todos",
  "codelogicx_task_manager_lookups",
  "codelogicx_task_manager_activity"
] as const;

type DynamicDatabase = Record<string, Record<string, unknown>>;

export class CodeLogicXSyncRepository {
  constructor(
    private readonly database: Kysely<CodeLogicXDatabase> = getCodeLogicXDatabase(),
    private readonly attachmentStorage = new ProjectManagerAttachmentStorage()
  ) {}

  async exportSnapshot(instanceId: string): Promise<CodeLogicXSyncSnapshot> {
    ...
    const attachmentRows =
      (Object.fromEntries(entries)["codelogicx_project_manager_attachments"] as
  ...
  async exportProjectSnapshot(instanceId: string): Promise<CodeLogicXSyncSnapshot> {
    ...
    return {
      ...
      publishedAt: new Date().toISOString(),
      tables: {
        codelogicx_project_manager_items: rows.map(({ id: _id, ...row }) => serializable(row))
      }
    };
  }
  ...
  async importSnapshot(snapshot: CodeLogicXSyncSnapshot) {
    let records = 0;
  ...
  private async importAttachments(snapshot: CodeLogicXSyncSnapshot) {
    const rows = snapshot.tables.codelogicx_project_manager_attachments ?? [];
  ...
  async createToken(input: { actor: string; hash: string; label: string }) {
    const uuid = newUuid();
    await this.database
      .insertInto("codelogicx_sync_tokens")
      .values({
  ...
  listTokens() {
    return this.database
      .selectFrom("codelogicx_sync_tokens")
  ...
  async revokeToken(uuid: string) {
    const result = await this.database
      .updateTable("codelogicx_sync_tokens")
      .set({ status: "revoked" })
      .where("uuid", "=", uuid)
      .where("status", "=", "active")
      .executeTakeFirst();
  ...
  connection() {
    return this.database
      .selectFrom("codelogicx_sync_connections")
      .selectAll()
  ...
  async saveConnection(input: { encryptedToken: string; instanceId: string }) {
    await this.database
      .insertInto("codelogicx_sync_connections")
      .values({
        ...
        server_url: "https://codelogicx.codexsun.com",
        status: "bound"
      })
  ...
  async updateConnection(input: {
    ...
    await this.database
      .updateTable("codelogicx_sync_connections")
      .set({
  ...
  async deleteConnection() {
    await this.database
      .deleteFrom("codelogicx_sync_connections")
      .where("server_id", "=", "codexsun-cloud")
      .executeTakeFirst();
  }
  ...
  latestSnapshot() {
    return this.database
      .selectFrom("codelogicx_sync_snapshots")
      .selectAll()
      .where("server_id", "=", "codexsun-cloud")
  ...
  async saveSnapshot(input: {
    ...
    await this.database
      .insertInto("codelogicx_sync_snapshots")
      .values({
  ...
  async conflictCount() {
    const row = await this.database
      .selectFrom("codelogicx_sync_conflicts")
  ...
  async startRun(direction: "pull" | "push", revision: number) {
    const uuid = newUuid();
    await this.database
      .insertInto("codelogicx_sync_runs")
      .values({
        direction,
        error_message: null,
  ...
  async finishRun(
    ...
    await this.database
      .updateTable("codelogicx_sync_runs")
      .set({
  ...
  async recordConflict(input: {
    ...
    await this.database
      .insertInto("codelogicx_sync_conflicts")
      .values({
        ...
        status: "open",
        table_name: "codelogicx_sync_snapshot",
        uuid: newUuid()
      })
      .executeTakeFirstOrThrow();
  }

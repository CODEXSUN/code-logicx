...
import { CodeLogicXSyncRepository, synchronizedTables } from "./sync.repository.js";
import {
  CODELOGICX_SYNC_CLOUD_URL,
  type CodeLogicXSyncResult,
  type CodeLogicXSyncRole,
  type CodeLogicXSyncSnapshot,
  type CodeLogicXSyncStatus,
  type CodeLogicXSyncTokenSummary
} from "./sync.types.js";

type CloudSnapshotEnvelope = {
  checksum: string;
  revision: number;
  snapshot: CodeLogicXSyncSnapshot;
};

export class CodeLogicXSyncService {
  constructor(private readonly repository = new CodeLogicXSyncRepository()) {}

  role(): CodeLogicXSyncRole {
    const role = process.env.CODELOGICX_SYNC_ROLE?.trim().toLowerCase() || "local";
    if (!["cloud", "disabled", "local"].includes(role)) {
      throw new Error("CODELOGICX_SYNC_ROLE must be cloud, local, or disabled.");
    }
    return role as CodeLogicXSyncRole;
  }

  async status(): Promise<CodeLogicXSyncStatus> {
    ...
    return {
      ...
      cloudUrl: CODELOGICX_SYNC_CLOUD_URL,
      conflictCount: conflicts,
      instanceId: connection?.instance_id ?? process.env.CODELOGICX_SYNC_INSTANCE_ID?.trim() ?? "",
  ...
  async cloudTokens(): Promise<CodeLogicXSyncTokenSummary[]> {
    this.requireRole("cloud");
  ...
  async publishProjects(acceptLocal: boolean, acceptRemote: boolean): Promise<CodeLogicXSyncResult> {
  ...
  async publish(): Promise<CodeLogicXSyncResult> {
    this.requireRole("local");
    const connection = await this.requiredConnection();
    const run = await this.repository.startRun("push", connection.remote_revision);
    try {
  ...
  async pull(): Promise<CodeLogicXSyncResult> {
    this.requireRole("local");
    const connection = await this.requiredConnection();
  ...
  async authenticateCloudToken(token: string) {
    ...
    if (!record) throw AppError.unauthorized("CodeLogicX sync token is invalid.");
  ...
  async cloudPublish(token: string, baseRevision: number, snapshot: CodeLogicXSyncSnapshot) {
  ...
  private async requiredConnection() {
    ...
    if (!connection)
      throw AppError.validation("Bind this CodeLogicX installation to the cloud before synchronizing.");
    return connection;
  }

  private requireRole(expected: CodeLogicXSyncRole) {
    if (this.role() !== expected) {
      throw AppError.forbidden(`CodeLogicX sync operation requires the ${expected} runtime role.`);
    }
  }
}
...
async function cloudRequest<T>(path: string, token: string, body?: unknown): Promise<T> {
  ...
  if (!response.ok || !result?.success || result.data === undefined) {
    throw new AppError({
      ...
      message:
        result?.error?.message ?? `CodeLogicX cloud synchronization failed (${response.status}).`,
...
function cloudUrl() {
  const testUrl =
    process.env.NODE_ENV === "test" ? process.env.CODELOGICX_SYNC_TEST_CLOUD_URL?.trim() : "";
  return `${testUrl || `${CODELOGICX_SYNC_CLOUD_URL}/api/codelogicx/sync/cloud`}`.replace(/\/+$/u, "");
}
...
function requiredToken(value: string) {
  ...
  if (!/^[A-Za-z0-9]{16}$/u.test(token))
    throw AppError.validation("CodeLogicX sync token must contain 16 characters.");
  return token;
}
...
function validateSnapshot(snapshot: CodeLogicXSyncSnapshot) {
  if (snapshot.protocolVersion !== 1)
    throw AppError.validation("CodeLogicX sync protocol version is unsupported.");
  for (const table of Object.keys(snapshot.tables)) {
    if (!synchronizedTables.includes(table as (typeof synchronizedTables)[number])) {
      throw AppError.validation(`CodeLogicX sync table is not allowed: ${table}.`);
    }
  }
}

function parseSnapshot(value: string) {
  return JSON.parse(value) as CodeLogicXSyncSnapshot;
}

...
function restoreCloudSession() { const token = localStorage.getItem(cloudTokenKey); const endpoint = localStorage.getItem(cloudEndpointKey); if (token) localStorage.setItem(tokenKey, token); else localStorage.removeItem(tokenKey); if (endpoint) localStorage.setItem(endpointKey, endpoint); else localStorage.removeItem(endpointKey); }

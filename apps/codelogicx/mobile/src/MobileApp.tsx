...
import { bulbOutline, chatbubbleEllipsesOutline, checkboxOutline, chevronDownOutline, chevronForwardOutline, closeOutline, folderOutline, homeOutline, layersOutline, logOutOutline, menuOutline, optionsOutline, settingsOutline } from "ionicons/icons";
import { useCallback, useEffect, useState } from "react";
import { mobileApi } from "./mobile-api";
import { startUpdateMonitor } from "./app-updater";
...
function MobileDesk({ onSignOut }: { onSignOut: () => void }) {
  ...
  return <main className={compact ? "mobile-desk is-compact" : "mobile-desk"}>
    ...
    <div className="page-scroll">{loading ? <LoadingPage/> : error ? <ErrorPage message={error} retry={() => void load()}/> : renderPage(tab, data, navigate, load, session?.userId ?? "")}</div>

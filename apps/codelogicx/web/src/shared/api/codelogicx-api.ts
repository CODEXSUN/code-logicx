...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!text)
    throw new Error(
      `CodeLogicX API returned an empty response (${response.status}).`,
    );
  ...
  try {
    ...
    throw new Error(
      `CodeLogicX API returned an invalid response (${response.status}).`,
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      envelope && !envelope.success
        ...
        : `CodeLogicX API download failed (${response.status}).`,
    );
  }

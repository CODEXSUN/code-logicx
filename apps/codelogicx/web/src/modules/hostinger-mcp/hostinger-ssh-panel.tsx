...
export function HostingerSshPanel({ node }: { node: HostingerNode }) {
  ...
  const [target, setTarget] = useState<HostingerSshTarget>({
    ...
    name: `codelogicx-codelogicx-${node.id}`,
    port: 22,
    user: "root",
    virtualMachineId: node.id
  });
  const statusKey = ["codelogicx", "hostinger-ssh", target] as const;
  ...
  return <section className="mt-6 rounded-2xl border bg-card shadow-sm">
    ...
    {result?.fingerprint || result?.lastError || result?.evidence ? <div aria-live="polite" className="mx-5 mb-5 rounded-xl bg-muted/50 p-4 text-sm"><div className="flex items-center gap-2 font-medium"><ShieldCheckIcon className="size-4 text-emerald-600" />Private key protected by the CodeLogicX server</div>{result.fingerprint ? <p className="break-all pt-2 text-xs text-muted-foreground">Fingerprint: {result.fingerprint}</p> : null}{result.connected && result.evidence ? <p className="pt-2 text-emerald-700">Connected to {result.evidence.host} as {result.evidence.user}. `/home/codelogicx` is {result.evidence.codelogicxPath}.</p> : null}{result.lastError ? <p className="pt-2 text-sm text-destructive">{result.lastError}</p> : null}</div> : null}
  </section>;
}

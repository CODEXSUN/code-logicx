...
- `launch-desk.services.ts` calls the CodeLogicX orchestration API.
...
3. Optionally set `CODELOGICX_CODEX_HOME` to choose the private authentication location.
4. Run `npm.cmd install` from the repository root.
5. Run `npm.cmd run dev`.
6. Open `/app/codelogicx/launch-desk`.

CodeLogicX defaults to `%LOCALAPPDATA%/CodeLogicX/CodeLogicX/codex`. Keep this directory outside source control. The connection page does not need `OPENAI_API_KEY` because it authenticates the local Codex runtime through the official device flow.
...
- [ ] CodeLogicX uses an authentication location independent from Codex IDE.

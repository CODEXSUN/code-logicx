...
export function LaunchDeskWorkspace() {
  ...
  const connections = useQuery({
    ...
    queryKey: ["codelogicx", "codex", "connections"],
    refetchInterval: loginPending ? 2_000 : 30_000
  });
  const providers = useQuery({
    queryFn: getModelProviders,
    queryKey: ["codelogicx", "model-providers"]
  });
  ...
  const refresh = () =>
    queryClient.invalidateQueries({ queryKey: ["codelogicx", "codex", "connections"] });
  ...
  const refreshProviders = () =>
    queryClient.invalidateQueries({ queryKey: ["codelogicx", "model-providers"] });

...
import { Button, Input } from "@codelogicx/ui";
    ...
    <WorkspacePage className="!w-full !max-w-none !space-y-0 !py-0" title="">
      <div className="grid min-h-[calc(100vh-64px)] overflow-hidden border-y bg-background md:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="flex min-h-0 flex-col border-r">
          <div className="flex items-center justify-between gap-3 border-b p-4"><div><h1 className="text-xl font-semibold">Messenger</h1><p className="text-sm text-muted-foreground">Your CodeLogicX conversations</p></div><Button aria-label="New conversation" size="icon" onClick={() => setShowContacts((value) => !value)}><Plus className="size-4" /></Button></div>
...
function currentActorId() { try { const token = window.localStorage.getItem("codelogicx_session") ?? ""; return String((JSON.parse(atob(token.split(".")[1] ?? "")) as { userId?: string }).userId ?? ""); } catch { return ""; } }

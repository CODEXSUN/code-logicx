...
export function AgentIdeWorkspace() {
  ...
  const codexConnections = useQuery({
    queryKey: ["codelogicx", "agent-ide", "codex-connections"],
  ...
  useEffect(() => {
    if (projectId) window.localStorage.setItem("codelogicx_agent_project", projectId);
    else window.localStorage.removeItem("codelogicx_agent_project");
  }, [projectId]);
  ...
  const historyQuery = useQuery({
    queryKey: ["codelogicx", "agent-ide", "chat-history"],
  ...
  const send = async (text: string, attachments: AgentIdeAttachment[]) => {
    ...
    try {
      await streamAgentIdeChat(
        ...
        (event) => {
          if (event.type === "chat.started") {
            ...
            void queryClient.invalidateQueries({ queryKey: ["codelogicx", "agent-runs", project.id] });
          }
      ...
      setRunning(false);
      setActivity("");
      void queryClient.invalidateQueries({ queryKey: ["codelogicx", "agent-ide", "chat-history"] });
    }
  };
      ...
      <header className="relative z-20 flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3">
        ...
        <div className="flex min-w-0 flex-1 items-center justify-end gap-3 sm:max-w-2xl">
          ...
          <Button asChild className="gap-2" size="sm" variant="ghost">
            <a href="/app/codelogicx/honey" title="Open Honey Chat">
...
function storedProjectId() {
  return window.localStorage.getItem("codelogicx_agent_project") ?? "";
}

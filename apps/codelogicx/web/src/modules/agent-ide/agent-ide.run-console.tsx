...
export function AgentIdeRunConsole({
  ...
  const runsQuery = useQuery({
    ...
    queryKey: ["codelogicx", "agent-runs", projectUuid],
  ...
  const detailQuery = useQuery({
    ...
    queryKey: ["codelogicx", "agent-run", selectedId],
    refetchInterval: 2_000
  });
  ...
  const cleanup = async () => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ["codelogicx", "agent-run", run.uuid] });
      await queryClient.invalidateQueries({ queryKey: ["codelogicx", "agent-runs", projectUuid] });
    } catch (error) {

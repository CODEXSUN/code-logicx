...
import { WorkspaceFilters } from "@codelogicx/ui/workspace/filters";
import { WorkspaceLookup, type WorkspaceLookupOption } from "@codelogicx/ui/workspace/lookup";
import { WorkspaceMinimalEditor } from "@codelogicx/ui/workspace/minimal-editor";
import { WorkspacePage } from "@codelogicx/ui/workspace/page";
import { WorkspacePagination } from "@codelogicx/ui/workspace/pagination";
...
import { buildShowingLabel } from "@codelogicx/ui/workspace/utils";
import {
...
import { openRecordWhiteboards, ProjectWhiteboards } from "../planning/planning.record-whiteboards";
import {
  RoadmapStatistics,
...
const projectTypeOptions = ["product", "client", "internal", "research", "maintenance"];
...
export function WorkAutomationWorkspace({
  ...
  const [projectDashboard, setProjectDashboard] = useState(false);
  ...
  const dashboardProject = rootProject
    ? ((projectQuery.data ?? []).find((record) => record.id === rootProject.id) ?? rootProject)
    : null;
  ...
  const rootProjectsView = !parent && !forcedKind && kind === "project";
  const parentKind: FlowKind =
    parent && flow.includes(parent.kind as FlowKind) ? (parent.kind as FlowKind) : "project";
  ...
  const workflowRecords = useMemo(
    ...
    [activityQuery.data, projectQuery.data, query.data, reviewQuery.data, taskQuery.data]
  );
  ...
  const isolatedWorkflow = useMemo(
    () =>
      workflowSearchResult
        ? isolateWorkflow(workflowSearchResult, workflowRecords, initialView === "roadmap")
        : null,
  ...
  useEffect(() => {
    ...
    if (reviewParentId) {
      ...
      const targetKey = targetActivity ? `drill-reviews:${targetActivity.id}` : "";
      if (targetActivity && openedRecord.current !== targetKey) {
        openedRecord.current = targetKey;
        setPath([...buildParentPath(targetActivity, workflowRecords), targetActivity]);
        setForcedKind(null);
        setEditing(null);
        setSearch("");
        setStatusFilter("all");
        setPage(1);
        window.history.replaceState({ page: "projects" }, "", "/app/codelogicx/projects");
      }
      return;
    }
    ...
    if (drillIssueId) {
      ...
      const targetIssue = workflowRecords.find(
        (record) =>
          record.kind === "issue" && (record.id === drillIssueId || record.key === drillIssueId)
      );
      const targetKey = targetIssue ? `drill-issue:${targetIssue.id}` : "";
      if (targetIssue && openedRecord.current !== targetKey) {
        openedRecord.current = targetKey;
        setPath([...buildParentPath(targetIssue, workflowRecords), targetIssue]);
        setForcedKind(null);
        setEditing(null);
        setSearch("");
        setStatusFilter("all");
    ...
    if (drillProjectId) {
      ...
      const targetKey = targetProject ? `drill-project:${targetProject.id}` : "";
      if (targetProject && openedRecord.current !== targetKey) {
        openedRecord.current = targetKey;
        setPath([targetProject]);
        setProjectDashboard(params.get("view") !== "issues");
        setForcedKind(null);
        setEditing(null);
        setSearch("");
        setStatusFilter("all");
        ...
        window.history.replaceState(
          { page: "project-dashboard", projectId: targetProject.id },
          "",
          `/app/codelogicx/projects?project=${encodeURIComponent(targetProject.id)}`
        );
      }
      return;
    }
  ...
  useEffect(() => {
    ...
    const restoreProjectRoute = () => {
      ...
      const project = (projectQuery.data ?? []).find(
        (record) => record.id === projectId || record.key === projectId
      );
      if (!project) return;
      setPath([project]);
      setProjectDashboard(params.get("view") !== "issues");
  ...
  }, [projectQuery.data, workflowOnly]);
  ...
  function lookupOptions(lookupKind: LookupKind) {
    ...
    return uniqueOptions([...values.map(toOption), ...createdOptions[lookupKind]]);
  }
  ...
  function openIssueDelivery(view: "gantt" | "roadmap") {
    ...
    if (!issue) return;
    const reviewParentId = kind === "review" && parent?.kind === "activity" ? parent.id : "";
    window.location.assign(
      `/app/codelogicx/roadmap?issue=${encodeURIComponent(issue.id)}${
        reviewParentId ? `&reviewParent=${encodeURIComponent(reviewParentId)}` : ""
  ...
  function backToReviews() {
    ...
    if (reviewParentId) {
      window.location.assign(
        `/app/codelogicx/projects?reviewParent=${encodeURIComponent(reviewParentId)}`
      );
      return;
    }
    window.history.back();
  ...
  function selectRoadmapView(stage: DeliveryStage) {
    if (stage === "roadmap" || stage === "gantt") {
      ...
      if (roadmapIssue) {
        ...
        window.history.replaceState(
          ...
          "",
          `/app/codelogicx/roadmap?issue=${encodeURIComponent(roadmapIssue.id)}${
            reviewParentId ? `&reviewParent=${encodeURIComponent(reviewParentId)}` : ""
          }${view === "gantt" ? "&view=gantt" : ""}`
        );
      }
    ...
    if (roadmapIssue && stage === "issue") {
      window.location.assign(`/app/codelogicx/projects?issue=${encodeURIComponent(roadmapIssue.id)}`);
    }
  }
      ...
      actions={
        workflowOnly ? (
          <div className="flex items-center gap-2">
            ...
            {roadmapIssue ? (
              <WorkspaceStatusBadge
                ...
                tone={roadmapIssue.active ? statusTone(roadmapIssue.status) : "neutral"}
              />
            ) : null}
            <Button
              variant="outline"
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" onClick={() => openEditor(dashboardProject)}>
      ...
      {workflowOnly ? (
                ...
                <div className="min-w-0">
                  <div className="mb-4 flex flex-wrap gap-2 rounded-md border bg-card p-2 shadow-sm">
                    {(initialView === "roadmap"
                      ...
                      : (["timeline", "gantt", "reviews", "kanban"] as WorkflowView[])
                    ).map((view) => (
                      <Button
                        key={view}
                        size="sm"
        ...
        <ProjectDashboard
          ...
          onOpenIssues={() => {
            setProjectDashboard(false);
            window.history.replaceState(
              { page: "project-issues", projectId: dashboardProject.id },
              "",
          ...
          {rootProjectsView ? (
            <div className="mt-4">
              <ProjectCardList
                ...
                onWhiteboards={openRecordWhiteboards}
              />
            </div>
          ) : (
                          ...
                          <td className="px-4 py-3">
                            ...
                            {nextKind ? (
                              <div className="mt-1 inline-flex items-center gap-1 text-xs text-muted-foreground">
                                ...
                                {childCount(queries[nextKind].data ?? [], record)}{" "}
                                {plural(nextKind)}
                              </div>
                              ...
                              <WorkspaceRowActions
                                ...
                                actions={[
                                  ...
                                  },
                                  {
                                    ...
                                    onSelect: () => openRecordWhiteboards(record)
                                  },
                                  {
                                    id: "edit",
                                    label: "Edit",
      ...
      <div className="flex min-w-max items-center gap-1">
        {stages.map((stage, index) => (
          <div className="flex items-center gap-1" key={stage.id}>
            {index ? <ChevronRightIcon className="size-4 text-muted-foreground/60" /> : null}
            <Button
            ...
            >
              ...
              {stage.id === "gantt" ? <ChartNoAxesGanttIcon className="size-4" /> : null}
              {stage.label}
            </Button>
          </div>
        ))}
    ...
    <WorkspaceUpsertDialog
      ...
      description={
        parent
          ...
          : kind === "project"
            ? "Define the ownership, delivery dates, intended outcome, and card appearance."
            : `Capture the ${kind} details required for the project delivery flow.`
      }
      open
      onClose={onCancel}
      title={`${form.id ? "Edit" : "New"} ${label(kind)}`}
    >
      <form
        noValidate
        onSubmit={(event) => {
          ...
          <div className="grid gap-5">
            ...
            {kind === "project" ? (
              <div className="grid grid-cols-2 gap-3">
                <WorkspaceFormField label="Card logo" required>
                  <Input
                    aria-invalid={invalid.has("card logo")}
                    className="font-semibold uppercase"
                    maxLength={4}
                    placeholder="CX"
                    value={form.logoText}
                    onChange={(event) => patch("logoText", event.target.value.toUpperCase())}
                  />
                </WorkspaceFormField>
                <WorkspaceFormField label="Card color" required>
                  <WorkspaceSelect
                    value={form.colorKey}
                    options={projectColorOptions}
                    onValueChange={(value) => patch("colorKey", value)}
                  />
                </WorkspaceFormField>
              </div>
            ) : null}
            ...
            {kind === "project" || kind === "issue" || kind === "task" ? (
              <WorkspaceFormField label="Planned start" required={kind === "project"}>
                <WorkspaceDatePicker
...
function requiredFields(form: ProjectManagerForm, kind: FlowKind): string[] {
  ...
  if (kind === "project") {
    ...
    fields.push(["Card color", form.colorKey]);
  }

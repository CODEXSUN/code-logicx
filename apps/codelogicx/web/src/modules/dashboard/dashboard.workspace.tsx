  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-6 py-8 lg:px-8">
      ...
      <section className="grid grid-cols-2 border-b py-6 sm:grid-cols-4">
        ...
        />
        <Metric
          href="/app/codelogicx/github?view=checks"
          label="Builds failed"
          value={today?.failedChecks.length ?? 0}
        />
        <Metric
          href="/app/codelogicx/releases"
      ...
      <div className="grid gap-x-12 gap-y-9 pb-8 pt-16 lg:grid-cols-2">
        ...
        <Section href="/app/codelogicx/github" title="Engineering health">
          <div className="divide-y rounded-lg border bg-card">
            ...
            <HealthRow
              ...
              href="/app/codelogicx/github?view=sync"
            ...
            <HealthRow
              ...
              href="/app/codelogicx/github?view=working-trees"
              label="Working trees"
              value={
        ...
        <Section href="/app/codelogicx/projects" title="Projects">
          <div className="divide-y rounded-lg border bg-card">
            {projects.length ? (
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 flex gap-4 text-sm font-medium text-primary">
              <a className="hover:underline" href="/app/codelogicx/my-work">
                Review work
              </a>
              <a
                className="inline-flex items-center gap-1 hover:underline"
                href="/app/codelogicx/agent-ide"
              >
...
function ProjectRow({ project }: { project: ProjectManagerRecord }) {
  return (
    <a
      className="flex items-center gap-3 px-4 py-3 hover:bg-muted/40"
      href={`/app/codelogicx/projects?project=${encodeURIComponent(project.id)}`}
    >
      <span className="min-w-0 flex-1">
...
function recordHref(record: Pick<TodayRecord, "id" | "kind">) {
  ...
  if (record.kind === "task") return `/app/codelogicx/tasks?task=${id}`;
  if (record.kind === "issue") return `/app/codelogicx/issues?issue=${id}`;
  if (record.kind === "release") return `/app/codelogicx/releases?release=${id}`;
  if (record.kind === "project") return `/app/codelogicx/projects?project=${id}`;
  if (record.kind === "review") return `/app/codelogicx/my-work?view=reviews&record=${id}`;
  return `/app/codelogicx/my-work?view=activity&record=${id}`;
}

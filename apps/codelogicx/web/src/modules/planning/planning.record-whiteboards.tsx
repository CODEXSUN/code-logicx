      ...
      <div className="flex flex-wrap items-center gap-2 text-xs">
        ...
        {recent ? (
          <button
            ...
            type="button"
            onClick={() =>
              window.location.assign(`/app/codelogicx/planning/${recent.uuid}`)
            }
          >
            Open recent
          </button>
...
export function openRecordWhiteboards(record: ProjectManagerRecord) {
  window.location.assign(
    `/app/codelogicx/planning?recordKind=${encodeURIComponent(record.kind)}&recordUuid=${encodeURIComponent(record.id)}`,
  );
}

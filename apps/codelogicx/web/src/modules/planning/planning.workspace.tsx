...
function PlanningBoardList() {
  ...
  const create = async () => {
    ...
    window.location.assign(`/app/codelogicx/planning/${board.uuid}`);
  };
  return (
        ...
        <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {(boards.data ?? []).map((board, index) => (
            <button
              ...
              onClick={() => window.location.assign(`/app/codelogicx/planning/${board.uuid}`)}
            >
...
function PlanningEditor({ uuid }: { uuid: string }) {
  ...
  const leave = async () => {
    if (timer.current) clearTimeout(timer.current);
    if (await save()) window.location.assign("/app/codelogicx/planning");
  };
  const remove = async () => {
    await actions.delete.mutateAsync(uuid);
    window.location.assign("/app/codelogicx/planning");
  };

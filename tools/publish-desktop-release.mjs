...
const targetRoot = join(root, "apps", "codelogicx", "desktop", "src-tauri", "target", "release");
...
async function publishDesktopRelease() {
  ...
  const files = [
    copyArtifact(join(targetRoot, "CodeLogicX.exe"), join("app", "CodeLogicX.exe"), "app"),
  ...
  try {
    ...
    execFileSync(
      ...
      ],
      {
        env: {
          ...process.env,
          CODELOGICX_MSI_PATH: installerPath,
          CODELOGICX_VERSION: version
        },
        stdio: "inherit"
      }
    );
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      VALUE "FileDescription", "CodeLogicX Setup\\0"
      VALUE "FileVersion", "${version}\\0"
      VALUE "InternalName", "CodeLogicX Setup\\0"
      VALUE "OriginalFilename", "${setupName}\\0"
      VALUE "ProductName", "CodeLogicX\\0"

...
const INSTALLER_BYTES: &[u8] = include_bytes!(env!("CODELOGICX_MSI_PATH"));
const VERSION: &str = env!("CODELOGICX_VERSION");
...
fn install() -> io::Result<()> {
    ...
    let installer = directory.join(format!("CodeLogicX_{VERSION}_x64_en-US.msi"));
...
fn create_installer_directory() -> io::Result<PathBuf> {
    ...
    let directory = std::env::temp_dir().join(format!(
        "CodeLogicX-setup-{VERSION}-{}-{timestamp}",
        process::id()
    ));
...
fn show_error(message: &str) {
    ...
    let title = wide("CodeLogicX Setup");
    unsafe {
        MessageBoxW(
            std::ptr::null_mut(),
            message.as_ptr(),

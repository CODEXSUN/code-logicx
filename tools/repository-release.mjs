...
const tauriConfigPath = join(root, "apps", "codelogicx", "desktop", "src-tauri", "tauri.conf.json");
const cargoManifestPath = join(root, "apps", "codelogicx", "desktop", "src-tauri", "Cargo.toml");
const cargoLockPath = join(root, "apps", "codelogicx", "desktop", "src-tauri", "Cargo.lock");
...
function checkVersions() {
  ...
  if (existsSync(cargoManifestPath)) {
    checkOwnedVersion(
      failures,
      cargoManifestPath,
      rustPackageVersion(readFileSync(cargoManifestPath, "utf8"), "codelogicx-desktop"),
      expected
    );
  }
  if (existsSync(cargoLockPath)) {
    checkOwnedVersion(
      failures,
      cargoLockPath,
      rustPackageVersion(readFileSync(cargoLockPath, "utf8"), "codelogicx-desktop"),
      expected
    );
  }
...
function updateDesktopVersions(currentVersion, nextVersion) {
  ...
  updateRustPackageVersion(cargoManifestPath, "codelogicx-desktop", currentVersion, nextVersion);
  updateRustPackageVersion(cargoLockPath, "codelogicx-desktop", currentVersion, nextVersion);
}
  ...
  for (const file of packageFiles()) {
    ...
    for (const field of [
      ...
      for (const [name, value] of Object.entries(manifest[field] ?? {})) {
        if (!name.startsWith("@codelogicx/") && !name.startsWith("@codelogicx/")) continue;
...
function isVersionedInternalDependency(name, value) {
  return (
    (name.startsWith("@codelogicx/") || name.startsWith("@codelogicx/")) &&
    /^\^?\d+\.\d+\.\d+$/u.test(value)
  );
}
...
function askWindowsModal(question, defaultValue = "") {
  ...
  if (confirmation) {
    const script = [
      ...
      )}, 'CodeLogicX GitHub Release', 'YesNo', 'Question')`,
  ...
  const script = [
    ...
    `[Microsoft.VisualBasic.Interaction]::InputBox(${quotePowerShell(
      question
    )}, 'CodeLogicX GitHub Release', ${quotePowerShell(defaultValue)})`
  ].join("; ");
  return powershellModal(script);
}

...
console.info(
  "Runtime smoke passed: two API boots, single-client identity, CodeLogicX orchestration/project/task routes, and logout."
);
...
async function smokeCycle(cycle) {
  ...
  try {
    ...
    assert.ok(login.body.data?.permissions?.includes("codelogicx.project-manager.view"));
    assert.ok(login.body.data?.permissions?.includes("codelogicx.orchestration.view"));
    ...
    assert.ok(session.body.data?.permissions?.includes("codelogicx.project-manager.view"));
    assert.ok(session.body.data?.permissions?.includes("codelogicx.orchestration.view"));

    const orchestration = await request(baseUrl, "/api/codelogicx/orchestration/catalog", {
      headers: { authorization: `Bearer ${login.body.data.accessToken}` }
    });
    ...
    assert.equal(orchestration.body.data?.technicalName, "codelogicx");
    assert.equal(orchestration.body.data?.agentProfiles?.length, 6);
    assert.equal(orchestration.body.data?.assistModes?.length, 9);

    const tools = await request(baseUrl, "/api/codelogicx/orchestration/agent-ide/tools", {
      headers: { authorization: `Bearer ${login.body.data.accessToken}` }
    });
    ...
    const verificationCommands = await request(baseUrl, "/api/codelogicx/orchestration/agent-ide/verification/commands", {
      headers: { authorization: `Bearer ${login.body.data.accessToken}` }
    });
    ...
    const projects = await request(baseUrl, "/api/codelogicx/admin/project-manager/result", {
      headers: { authorization: `Bearer ${login.body.data.accessToken}` }
    });
    ...
    const todos = await request(baseUrl, "/api/codelogicx/task-manager/todos", {
      headers: { authorization: `Bearer ${login.body.data.accessToken}` }
    });

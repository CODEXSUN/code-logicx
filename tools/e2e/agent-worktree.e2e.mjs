...
const modulePath = resolve(
  root,
  "dist/codelogicx/api/modules/orchestration/agent-worktree.service.js"
);
const budgetModulePath = resolve(
  root,
  "dist/codelogicx/api/modules/orchestration/agent-run.budget.js"
);
const verificationModulePath = resolve(
  root,
  "dist/codelogicx/api/modules/orchestration/agent-verification.service.js"
);
const integrationModulePath = resolve(
  root,
  "dist/codelogicx/api/modules/orchestration/agent-integration.service.js"
);
const fingerprintModulePath = resolve(
  root,
  "dist/codelogicx/api/modules/orchestration/agent-workspace-fingerprint.js"
);
const temporaryRoot = await mkdtemp(join(tmpdir(), "codelogicx-agent-worktree-"));
...
try {
  ...
  process.env.CODELOGICX_AGENT_ALLOWED_ROOTS = temporaryRoot;
  process.env.CODELOGICX_AGENT_WORKTREE_ROOT = worktrees;
  ...
  const unavailableCommand = await runner.run(workspace.path, {
    args: [],
    command: "codelogicx-command-that-does-not-exist",
    id: "missing",

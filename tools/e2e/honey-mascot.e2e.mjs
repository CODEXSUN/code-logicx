...
test("Honey remains visible through idle, moves naturally, and supports placement", async ({
  ...
  await page.addInitScript(() => {
    ...
    window.localStorage.setItem("codelogicx.screen-companion.visible", "true");
    window.sessionStorage.setItem("codelogicx.screen-companion.honey-introduced", "true");
    if (!window.localStorage.getItem("codelogicx.screen-companion.position")) {
      window.localStorage.setItem(
        "codelogicx.screen-companion.position",
        JSON.stringify({ x: 64, y: 420 })
      );
    }
    window.localStorage.setItem("codelogicx.screen-companion.behavior", "stay");
  });
  await page.route("**/api/codelogicx/honey/**", async (route) => {
  ...
  await page.goto("/app/codelogicx/overview");
...
test("Honey submits a completed voice turn and reacts without roaming", async ({ page }) => {
  await page.addInitScript(() => {
    ...
    window.localStorage.setItem("codelogicx.screen-companion.behavior", "roam");
  });
  const now = new Date().toISOString();
    ...
    messages: [
      { body: "Show my next task", createdAt: now, id: "user-voice", role: "user" },
      {
        actions: [
          ...
          { href: "/app/codelogicx/agent-ide", id: "start-agent", label: "Start Project Agent" },
          { href: "/app/codelogicx/projects", id: "open-project", label: "Open project" }
        ],
  ...
  await page.goto("/app/codelogicx/honey");

...
const privateKeyPath = join(directory, "github_codelogicx");
const publicKeyPath = `${privateKeyPath}.pub`;
const comment = options.comment ?? `codelogicx-server@${hostname()}`;
...
function printHelp() {
  output.log(`Generate a temporary ED25519 keypair for CodeLogicX server access to GitHub.

Usage:
  npm run github:ssh-key
  npm run github:ssh-key -- --comment "codelogicx-server"

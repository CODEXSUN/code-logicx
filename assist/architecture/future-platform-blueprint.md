# CodeLogicX Future Platform Blueprint
...
This blueprint records the intended evolution of CodeLogicX into a local-first, hybrid developer and
engineering orchestration platform. CodeLogicX remains the external product label; `codelogicx`
...
- CodeLogicX is a standalone, local-authenticated, single-client modular monolith.
- Platform owns identity, executable API and web servers, composition, and one MariaDB connection.
- CodeLogicX modules own engineering product behavior.
- Browser and API routes remain under `/app/codelogicx/*` and `/api/codelogicx/*`.
...
CodeLogicX should become an engineering operating platform where developers, projects, source code,
...
```mermaid
flowchart TB
  user["Developer or Team"] --> web["CodeLogicX Web or Desktop"]
  web --> control["CodeLogicX Cloud Control Plane"]
  ...
  desktop --> localRuntime["codelogicx-node: Docker, Git, Browser, Local Models"]
  vps --> remoteRuntime["codelogicx-node: Workers and Docker"]
  gpu --> gpuRuntime["codelogicx-node: Accelerated Models"]
...
| Execution     | Application server and guarded deployment tooling | Distributed `codelogicx-node` execution network                  |
...
```text
React UI -> Tauri boundary -> CodeLogicX local runtime -> Docker, Git, agents, models, browser, OS
```

Each capable machine runs `codelogicx-node`, which will:
...
```text
Issue one-time registration token -> Install codelogicx-node -> Register
...
Create the minimal `codelogicx-node`, unique node registration, heartbeat, capability reporting,

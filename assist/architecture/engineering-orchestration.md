...
CodeLogicX is the external product and brand label. `codelogicx` is the internal technical name and is
not renamed. The stable technical contract includes workspace packages, `/app/codelogicx/*` browser
routes, `/api/codelogicx/*` API routes, `codelogicx.*` permissions, `CODELOGICX_*` environment keys, database
...
composition. CodeLogicX modules own all engineering product behavior.
...
- the Engineering Command Center combines live project, task, review, repository, and check
  signals already owned by CodeLogicX;
...
- the executor limits repository roots through `CODELOGICX_AGENT_ALLOWED_ROOTS`;
- CodeLogicX interrupts Codex when a run exceeds its time, tool, file, or sub-agent budget;
- reviewers can remove a clean terminal worktree while they keep its branch;
- CodeLogicX runs only registered quality-gate commands without a shell;
...
- a passed run requires a separate human action before CodeLogicX creates a local commit;
- the commit gate rejects a worktree that changed after its last passed verification attempt;
- CodeLogicX does not push Agent commits or change protected branches;
...
- CodeLogicX can register the Hostinger VPS MCP server in its private Codex runtime. The server-side
  environment supplies the API token, while Codex applies MCP tool approval policy. CodeLogicX never

---
name: codelogicx-production-update
description: Safely release and operate CodeLogicX at /home/codelogicx on its Ubuntu Docker VPS. Use when preparing a CodeLogicX version, checking SSH production state, running the guarded Docker updater, installing or diagnosing the systemd update watcher, verifying migrations and health, cleaning CodeLogicX-only one-off resources, or recording deployment evidence.
---

# CodeLogicX Production Update
...
5. Connect to the pinned VPS host key. Inspect `/home/codelogicx`, Docker, disk space,
...
2. Open an SSH session without echoing `.env` values. Verify host key, `/home/codelogicx`, branch, clean status, disk, and Compose state.
...
- The timer may fetch and deploy only `origin/main` into `/home/codelogicx`.
...
- Cleanup is limited to stopped Compose one-off containers from the configured
  CodeLogicX project and unused images in the configured CodeLogicX image namespace.

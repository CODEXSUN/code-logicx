...
- `apps/platform/api`: Fastify server, local authentication, identity modules, MariaDB connection,
  and CodeLogicX API composition.
- `apps/platform/web`: React application shell, login, identity administration, and CodeLogicX web
  bundle composition.

## CodeLogicX Owner Workspaces

- `apps/codelogicx/api`: Project Manager, Task Manager, Planning, GitHub Dashboard, Orchestration,
  Skills, Sync, Notifications, MariaDB lifecycle, and public host contracts.
- `apps/codelogicx/web`: Today, Projects, Tasks, Platform Registry, Whiteboards, GitHub Dashboard,
...
- `assist/skills/library`: Repository-owned physical skill folders used for Agent prompting and
  review workflows. CodeLogicX generates the hidden `SKILL.md` manifest and links each user-managed
  reference file from it so agents can load the relevant knowledge progressively.
- `apps/codelogicx/desktop`: Tauri and React local IDE with a bundled Codex runtime, Monaco, Zustand
...
- Sync owns encrypted local-to-cloud bindings, cloud acceptance tokens, revisioned snapshots,
  checksums, conflict records, and manual two-way transfer with `https://codelogicx.codexsun.com`.

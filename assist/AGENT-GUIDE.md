...
- CodeLogicX is standalone, local-authenticated, and single-client.
- CodeLogicX is the external label. `codelogicx` is the stable technical name for packages, routes,
...
- Platform owns users, roles, permissions, assignments, the API/web servers, and the database
  connection.
- CodeLogicX modules own their complete backend and frontend leaves, migrations, seeds, attachments,
  planning records, registry records, and synchronization records.
- Platform composes CodeLogicX only through the public `@codelogicx/codelogicx-api` and
  `@codelogicx/codelogicx-web` workspace contracts.
- Framework and UI are consumed only through their public package exports.

## Change Rules

- Preserve unrelated worktree changes.
- Keep product behavior inside its CodeLogicX module leaf.

...
RUNTIME_ENV="${CODELOGICX_RUNTIME_ENV:-$ROOT_DIR/.env}"
RUNTIME_ENV_EXAMPLE="$ROOT_DIR/.env.example"
DEPLOY_ENV="${CODELOGICX_DEPLOY_ENV:-$CONTAINER_DIR/deploy.env}"
...
Interactive standalone CodeLogicX container installation.
...
Configuration:
  .env                    CodeLogicX runtime and application settings
...
Included:
  ...
  - Repository-owned UI
  - CodeLogicX Platform API and Web
  - Reused existing Docker network and MariaDB, or CodeLogicX-owned infrastructure

Excluded:
  - CXApp, Billing, Mail, TMApp, Redis, Media, and all other stacks

The setup builds only this self-contained CodeLogicX repository. It asks whether
to reuse an existing Docker network and running MariaDB container or create a
dedicated CodeLogicX network and MariaDB. CodeLogicX uses only MariaDB and does not
inspect or depend on Redis, Media, or another application stack.
If dedicated CodeLogicX MariaDB data already exists, setup separately asks
whether to reuse or freshly recreate only the CodeLogicX database volume.
EOF
}
...
prepare_deploy_environment() {
  ...
  case "$(file_value "$DEPLOY_ENV" DB_USER)" in
    ""|root)
      set_file_value "$DEPLOY_ENV" DB_USER codelogicx
      echo "Configured the dedicated deployment database user: codelogicx"
      ;;
  esac
  set_default_if_empty "$DEPLOY_ENV" DB_NAME codelogicx_db
  ...
  set_file_value "$DEPLOY_ENV" CODELOGICX_VERSION "$version"
  set_file_value "$DEPLOY_ENV" CODELOGICX_IMAGE_TAG "$version"
  set_file_value "$DEPLOY_ENV" CODELOGICX_MIGRATION_COMPATIBLE_VERSION "$version"
  set_file_value "$DEPLOY_ENV" NODE_RUNTIME_VERSION "$node_version"
  set_file_value "$DEPLOY_ENV" NPM_RUNTIME_VERSION "$npm_version"
  set_default_if_empty "$DEPLOY_ENV" CODELOGICX_CODEX_STATE_VOLUME codelogicx-codex-state
  set_default_if_empty "$DEPLOY_ENV" CODELOGICX_AGENT_WORKTREE_VOLUME codelogicx-agent-worktrees
  set_default_if_empty "$DEPLOY_ENV" CODELOGICX_AGENT_REPOSITORY_VOLUME codelogicx-agent-repositories
...
configure_deploy_environment() {
  ...
  prompt_setting "$DEPLOY_ENV" CODELOGICX_COMPOSE_PROJECT "Compose project" codelogicx
  prompt_setting "$DEPLOY_ENV" CODELOGICX_IMAGE_REGISTRY "Image registry/prefix" codelogicx
  prompt_setting "$DEPLOY_ENV" CODELOGICX_API_CONTAINER_NAME "API container name" codelogicx-api
  prompt_setting "$DEPLOY_ENV" CODELOGICX_WEB_CONTAINER_NAME "Web container name" codelogicx-web
  prompt_setting "$DEPLOY_ENV" CODELOGICX_BIND_ADDRESS "Host bind address" 127.0.0.1
  prompt_setting "$DEPLOY_ENV" CODELOGICX_API_HOST_PORT "API host port" 9150
  prompt_setting "$DEPLOY_ENV" CODELOGICX_WEB_HOST_PORT "Web host port" 9160
  prompt_setting "$DEPLOY_ENV" DB_NAME "Application database name" codelogicx_db
  prompt_setting "$DEPLOY_ENV" DB_USER "Application database user" codelogicx
...
prepare_runtime_environment() {
  ...
  if [[ "$infrastructure_mode" == shared ]]; then
    database_host="$(file_value "$DEPLOY_ENV" CODELOGICX_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb)"
  else
  ...
  set_file_value "$RUNTIME_ENV" PLATFORM_API_PORT \
    "$(file_value "$DEPLOY_ENV" CODELOGICX_API_INTERNAL_PORT 9150)"
  set_file_value "$RUNTIME_ENV" PLATFORM_WEB_PORT \
    "$(file_value "$DEPLOY_ENV" CODELOGICX_WEB_HOST_PORT 9160)"
  set_file_value "$RUNTIME_ENV" CODELOGICX_DB_FRESH_ON_START 0
  set_file_value "$RUNTIME_ENV" CODELOGICX_DB_RESET_CONFIRM ""
  set_file_value "$RUNTIME_ENV" CODELOGICX_ALLOW_PRODUCTION_DB_RESET 0
...
configure_runtime_environment() {
  ...
  prompt_setting "$RUNTIME_ENV" INITIAL_ADMIN_EMAIL \
    "Initial administrator email" admin@codelogicx.in
...
select_infrastructure_mode() {
  ...
  while true; do
    read -r -p \
      "Reuse CXApp network and MariaDB, or create dedicated CodeLogicX infrastructure? [reuse/dedicated] " \
      answer
...
prepare_shared_infrastructure() {
  ...
  mariadb="$(file_value "$DEPLOY_ENV" CODELOGICX_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb)"
  network="$(file_value "$DEPLOY_ENV" CODELOGICX_NETWORK cxapp-network)"
  shared_env="$(file_value "$DEPLOY_ENV" CODELOGICX_SHARED_MARIADB_ENV_FILE)"
  ...
  if [[ -n "$shared_env" ]]; then
    ...
    set_file_value "$DEPLOY_ENV" CODELOGICX_SHARED_MARIADB_ROOT_PASSWORD "$root_password"
  ...
  root_password="$(file_value "$DEPLOY_ENV" CODELOGICX_SHARED_MARIADB_ROOT_PASSWORD)"
  [[ -n "$root_password" ]] || {
    echo "Shared MariaDB root password is unavailable." >&2
    echo "Configure CODELOGICX_SHARED_MARIADB_ROOT_PASSWORD in $DEPLOY_ENV." >&2
    exit 78
  }
}
...
configure_shared_infrastructure() {
  ...
  prompt_setting "$DEPLOY_ENV" CODELOGICX_SHARED_MARIADB_CONTAINER_NAME \
    "CXApp MariaDB container name" cxapp-mariadb
  prompt_setting "$DEPLOY_ENV" CODELOGICX_NETWORK "CXApp Docker network" cxapp-network
  set_file_value "$DEPLOY_ENV" CODELOGICX_NETWORK_EXTERNAL true
  prompt_setting "$DEPLOY_ENV" CODELOGICX_SHARED_MARIADB_ENV_FILE \
...
configure_dedicated_infrastructure() {
  ...
  echo
  echo "Dedicated CodeLogicX infrastructure settings"
  if [[ "$(file_value "$DEPLOY_ENV" CODELOGICX_NETWORK_EXTERNAL false)" == true ]]; then
    set_file_value "$DEPLOY_ENV" CODELOGICX_NETWORK codelogicx-network
  fi
  set_file_value "$DEPLOY_ENV" CODELOGICX_NETWORK_EXTERNAL false
  prompt_setting "$DEPLOY_ENV" CODELOGICX_MARIADB_CONTAINER_NAME \
    "Dedicated MariaDB container name" codelogicx-mariadb
  container="$(file_value "$DEPLOY_ENV" CODELOGICX_MARIADB_CONTAINER_NAME codelogicx-mariadb)"
  while docker container inspect "$container" >/dev/null 2>&1 &&
    ! container_is_compose_service "$container" \
      "$(file_value "$DEPLOY_ENV" CODELOGICX_COMPOSE_PROJECT codelogicx)" mariadb; do
    ...
    set_file_value "$DEPLOY_ENV" CODELOGICX_MARIADB_CONTAINER_NAME "$replacement"
    container="$replacement"
  done
  prompt_setting "$DEPLOY_ENV" CODELOGICX_NETWORK "Dedicated Docker network" codelogicx-network
  prompt_setting "$DEPLOY_ENV" CODELOGICX_MARIADB_DATA_VOLUME \
    "MariaDB data volume" codelogicx-mariadb-data
...
compose() {
  CODELOGICX_RUNTIME_ENV_FILE="$RUNTIME_ENV" docker compose \
...
verify_agent_runtime_image() {
  compose run --rm --no-deps api sh -lc '
    ...
    git --version
    test -w "$CODELOGICX_CODEX_HOME"
    test -w "$CODELOGICX_AGENT_WORKTREE_ROOT"
    test -w "$CODELOGICX_AGENT_ALLOWED_ROOTS"
  '
}

verify_agent_runtime_container() {
  local container
  container="$(file_value "$DEPLOY_ENV" CODELOGICX_API_CONTAINER_NAME codelogicx-api)"
...
ensure_available_host_port() {
  ...
  bind_address="$(file_value "$DEPLOY_ENV" CODELOGICX_BIND_ADDRESS 127.0.0.1)"
  while true; do
...
validate_deploy_environment() {
  ...
  for key in \
    ...
    CODELOGICX_API_CONTAINER_NAME \
    CODELOGICX_WEB_CONTAINER_NAME \
    CODELOGICX_MARIADB_CONTAINER_NAME \
    CODELOGICX_NETWORK \
    CODELOGICX_MARIADB_DATA_VOLUME \
    CODELOGICX_CODEX_STATE_VOLUME \
    CODELOGICX_AGENT_WORKTREE_VOLUME \
    CODELOGICX_AGENT_REPOSITORY_VOLUME; do
    safe_docker_name "$(file_value "$DEPLOY_ENV" "$key")"
  done
  ensure_available_host_port \
    CODELOGICX_API_HOST_PORT \
    "API host port" \
    "$(file_value "$DEPLOY_ENV" CODELOGICX_API_CONTAINER_NAME codelogicx-api)"
  ensure_available_host_port \
    CODELOGICX_WEB_HOST_PORT \
    "Web host port" \
    "$(file_value "$DEPLOY_ENV" CODELOGICX_WEB_CONTAINER_NAME codelogicx-web)"
}
...
select_database_mode() {
  ...
  while true; do
    read -r -p "Reuse or freshly recreate CodeLogicX MariaDB data? [reuse/fresh] " answer
    case "${answer:-reuse}" in
      reuse|Reuse|REUSE)
        printf 'reuse'
        return
        ;;
      fresh|Fresh|FRESH)
        read -r -p "Type FRESH $volume to delete only this CodeLogicX volume: " confirmation
...
reconcile_database_user() {
  ...
  if [[ "$infrastructure_mode" == shared ]]; then
    container="$(file_value "$DEPLOY_ENV" CODELOGICX_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb)"
    root_password="$(file_value "$DEPLOY_ENV" CODELOGICX_SHARED_MARIADB_ROOT_PASSWORD)"
  else
    container="$(file_value "$DEPLOY_ENV" CODELOGICX_MARIADB_CONTAINER_NAME codelogicx-mariadb)"
...
SQL
  then
    echo "Could not reconcile the CodeLogicX MariaDB application user." >&2
...
if [[ "$infrastructure_mode" == dedicated ]]; then
  database_volume="$(file_value "$DEPLOY_ENV" CODELOGICX_MARIADB_DATA_VOLUME codelogicx-mariadb-data)"
  safe_docker_name "$database_volume"
  database_mode="$(select_database_mode "$database_volume")"
  infrastructure_label="dedicated CodeLogicX network and MariaDB"
else
  database_volume=""
...
echo
echo "Standalone CodeLogicX deployment plan"
echo "  Runtime env: $RUNTIME_ENV"
echo "  Deploy env: $DEPLOY_ENV"
echo "  Source: self-contained CodeLogicX monorepo"
echo "  Infrastructure: $infrastructure_label"
echo "  MariaDB data: $database_mode"
echo "  Images: internal Framework/UI -> CodeLogicX Platform API/Web"
echo "  Runtime: CodeLogicX API, CodeLogicX Web, selected MariaDB"
...
read -r -p "Build and apply this standalone CodeLogicX installation? [Y/n] " confirmation
...
echo
echo "CodeLogicX standalone installation completed."
echo "Web: http://$(file_value "$DEPLOY_ENV" CODELOGICX_BIND_ADDRESS 127.0.0.1):$(file_value "$DEPLOY_ENV" CODELOGICX_WEB_HOST_PORT 9160)/"
echo "API health: http://$(file_value "$DEPLOY_ENV" CODELOGICX_BIND_ADDRESS 127.0.0.1):$(file_value "$DEPLOY_ENV" CODELOGICX_API_HOST_PORT 9150)/health"
echo "Agent repositories: /srv/codelogicx/repositories inside the API container"

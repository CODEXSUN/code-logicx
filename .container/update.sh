...
CHECK_ONLY=false
ALLOW_DIRTY=false
LOCK_FILE="${TMPDIR:-/tmp}/codelogicx-update.lock"
...
Safely update an existing CodeLogicX Docker installation while preserving:
...
if [[ "$CHECK_ONLY" != true ]]; then
  command -v flock >/dev/null 2>&1 || {
    echo "flock is required to serialize CodeLogicX deployment updates." >&2
    exit 69
  }
  exec 9>"$LOCK_FILE"
  flock -n 9 || {
    echo "Another CodeLogicX update is already running (lock: $LOCK_FILE)." >&2
    exit 75
  }
fi
...
validate_release_contract() {
  ...
  for key in CODELOGICX_VERSION CODELOGICX_IMAGE_TAG; do
  ...
  configured="$(file_value "$DEPLOY_ENV" CODELOGICX_MIGRATION_COMPATIBLE_VERSION)"
  [[ "$configured" == "$source_version" ]] || {
    echo "Migration compatibility is not approved for CodeLogicX $source_version." >&2
    echo "After confirming backward compatibility with the current image, set:" >&2
    echo "  CODELOGICX_MIGRATION_COMPATIBLE_VERSION=$source_version" >&2
...
inspect_source_state() {
  ...
  git -C "$ROOT_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1 || {
    echo "CodeLogicX update source is not a Git worktree: $ROOT_DIR" >&2
    exit 78
  }
  source_commit="$(git -C "$ROOT_DIR" rev-parse HEAD)"
  if [[ -n "$(git -C "$ROOT_DIR" status --porcelain --untracked-files=normal)" ]]; then
    source_dirty=true
    echo "WARNING: the CodeLogicX Git worktree contains uncommitted or untracked files." >&2
  ...
  cat >"${metadata_file}.partial" <<EOF
{
  ...
  "migrationCompatibilityVersion": "$(file_value "$DEPLOY_ENV" CODELOGICX_MIGRATION_COMPATIBLE_VERSION)",
...
for key in \
  ...
  CODELOGICX_NETWORK; do
  require_setting "$DEPLOY_ENV" "$key"
done

project="$(file_value "$DEPLOY_ENV" CODELOGICX_COMPOSE_PROJECT codelogicx)"
api_container="$(file_value "$DEPLOY_ENV" CODELOGICX_API_CONTAINER_NAME codelogicx-api)"
web_container="$(file_value "$DEPLOY_ENV" CODELOGICX_WEB_CONTAINER_NAME codelogicx-web)"
network="$(file_value "$DEPLOY_ENV" CODELOGICX_NETWORK cxapp-network)"
network_external="$(file_value "$DEPLOY_ENV" CODELOGICX_NETWORK_EXTERNAL false)"
image_registry="$(file_value "$DEPLOY_ENV" CODELOGICX_IMAGE_REGISTRY codelogicx)"
image_tag="$(file_value "$DEPLOY_ENV" CODELOGICX_IMAGE_TAG local)"
backup_retention="$(file_value "$DEPLOY_ENV" CODELOGICX_BACKUP_RETENTION 10)"
...
minimum_backup_mb="$(file_value "$DEPLOY_ENV" CODELOGICX_UPDATE_MIN_BACKUP_FREE_MB 1024)"
minimum_docker_mb="$(file_value "$DEPLOY_ENV" CODELOGICX_UPDATE_MIN_DOCKER_FREE_MB 5120)"
...
for setting in \
  "CODELOGICX_BACKUP_RETENTION:$backup_retention" \
  "CODELOGICX_UPDATE_MIN_BACKUP_FREE_MB:$minimum_backup_mb" \
  "CODELOGICX_UPDATE_MIN_DOCKER_FREE_MB:$minimum_docker_mb"; do
  key="${setting%%:*}"
  value="${setting#*:}"
...
for service_spec in "$api_container:api" "$web_container:web"; do
  ...
  docker container inspect "$container" >/dev/null 2>&1 || {
    echo "Existing CodeLogicX $service container was not found: $container" >&2
...
if [[ "$network_external" == true ]]; then
  mariadb_container="$(
    file_value "$DEPLOY_ENV" CODELOGICX_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb
  )"
  ...
  mariadb_container="$(
    file_value "$DEPLOY_ENV" CODELOGICX_MARIADB_CONTAINER_NAME codelogicx-mariadb
  )"
  safe_docker_name "$mariadb_container"
  docker container inspect "$mariadb_container" >/dev/null 2>&1 || {
    echo "Existing CodeLogicX MariaDB container was not found: $mariadb_container" >&2
    exit 69
  }
...
for container in "$mariadb_container" "$api_container" "$web_container"; do
  container_is_running "$container" || {
    echo "Existing CodeLogicX container is not running: $container" >&2
    exit 69
  }
done
...
if container_is_running "$api_container"; then
  docker exec "$api_container" node -e \
    "require('node:fs').accessSync(process.env.CODELOGICX_ENV_FILE_PATH, require('node:fs').constants.R_OK | require('node:fs').constants.W_OK)" \
    >/dev/null 2>&1 || {
...
echo
echo "CodeLogicX Docker update plan"
...
if [[ "$CHECK_ONLY" == true ]]; then
  echo
  echo "Existing CodeLogicX Docker deployment is ready to update."
  exit 0
fi

if [[ "$ASSUME_YES" != true ]]; then
  read -r -p "Build and update the existing CodeLogicX containers? [Y/n] " confirmation
...
api_url="http://${probe_address}:$(file_value "$DEPLOY_ENV" CODELOGICX_API_HOST_PORT 9150)/health"
web_url="http://${probe_address}:$(file_value "$DEPLOY_ENV" CODELOGICX_WEB_HOST_PORT 9160)/health"
...
echo
echo "CodeLogicX Docker update completed."

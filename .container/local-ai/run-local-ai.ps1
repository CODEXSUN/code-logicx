...
function Test-LocalAi([int]$OllamaPort, [int]$QdrantPort, [string]$Model, [switch]$KeepProbe) {
  ...
  $probeCollection = "codelogicx_setup_probe"
  ...
  Write-Host "Qdrant health and collection API succeeded$(if ($KeepProbe) { '; retained codelogicx_setup_probe.' } else { '; temporary probe removed.' })" -ForegroundColor Green
  ...
  $reply = Invoke-RestMethod -Method Post -Uri "$ollamaUrl/api/generate" -ContentType "application/json" -TimeoutSec 90 -Body (@{
      model = $Model
      prompt = "Reply with exactly: CodeLogicX local AI ready"
      stream = $false
    } | ConvertTo-Json)

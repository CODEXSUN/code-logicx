# CodeLogicX local AI environment

This operator kit creates a local-only Ollama and Qdrant environment when you explicitly apply it.
It does not start services, download models, alter CodeLogicX credentials, or change the default Codex
...
| CodeLogicX projects, identity, runs, and metadata | MariaDB | Existing transactional product store. |

MariaDB vector search is not added by this kit. Qdrant is a better local vector engine for the
future retrieval use case; MariaDB remains the source of truth for CodeLogicX records. The desktop
...
## CodeLogicX connection boundary
...
Qdrant is deliberately provisioned but not used by CodeLogicX yet. Add retrieval only after a measured
...
- The `test` command creates and removes only the `codelogicx_setup_probe` collection unless you pass
  `-KeepProbe`.

...
REMOTE="${CODELOGICX_WATCHER_REMOTE:-origin}"
BRANCH="${CODELOGICX_WATCHER_BRANCH:-main}"
DEPLOY_ENV="${CODELOGICX_WATCHER_DEPLOY_ENV:-$REPO_DIR/.container/deploy.env}"
STATE_DIR="${CODELOGICX_WATCHER_STATE_DIR:-/var/lib/codelogicx-update-watcher}"
LOCK_FILE="${CODELOGICX_WATCHER_LOCK_FILE:-/run/lock/codelogicx-update-watcher.lock}"
CHECK_ONLY=false

usage() {
  cat <<'EOF'
Usage: codelogicx-update-watcher.sh [--check]

Fetch and safely deploy a fast-forward update from origin/main. --check validates
the watcher, repository, Docker runtime, and current CodeLogicX deployment without
...
[[ -d "$REPO_DIR/.git" ]] || fail "CodeLogicX checkout was not found at $REPO_DIR"
...
candidate_dir="$(mktemp -d /var/tmp/codelogicx-update-candidate.XXXXXX)"
cleanup_candidate() {
...
for key in CODELOGICX_VERSION CODELOGICX_IMAGE_TAG CODELOGICX_MIGRATION_COMPATIBLE_VERSION; do
  grep -qE "^${key}=" "$DEPLOY_ENV" || fail "$key is missing from $DEPLOY_ENV"
  sed -i -E "s|^${key}=.*$|${key}=${version}|" "$DEPLOY_ENV"
done

log "Running deployment preflight for CodeLogicX $version"
bash "$REPO_DIR/update.sh" --check
log "Applying CodeLogicX $version"
bash "$REPO_DIR/update.sh" --yes

project="$(sed -n 's/^CODELOGICX_COMPOSE_PROJECT=//p' "$DEPLOY_ENV" | tail -n 1)"
image_registry="$(sed -n 's/^CODELOGICX_IMAGE_REGISTRY=//p' "$DEPLOY_ENV" | tail -n 1)"
project="${project:-codelogicx}"
image_registry="${image_registry:-codelogicx}"
...
log "Deployment completed for CodeLogicX $version at $target_commit"
